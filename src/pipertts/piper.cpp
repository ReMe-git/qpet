...
void textToWavBuffer(PiperConfig &config, Voice &voice, std::string text,
                   std::vector<char> &buffer, SynthesisResult &result) {
  std::vector<int16_t> audioBuffer;
  textToAudio(config, voice, text, audioBuffer, result, NULL); 
	
  auto synthesisConfig = voice.synthesisConfig;
	writeWavHeaderToBuffer(synthesisConfig.sampleRate, synthesisConfig.sampleWidth,
                 synthesisConfig.channels, (int32_t)audioBuffer.size(),
                 buffer);
	
	buffer.insert(buffer.end(), (const char *)(audioBuffer.data()),
			(const char *)(audioBuffer.data()) + sizeof(int16_t) * audioBuffer.size());
}
