...
Csm::csmBool LAppWavFileHandler::Update(Csm::csmFloat32 deltaTimeSeconds) {
  ...
  goalOffset = static_cast<Csm::csmUint32>(_userTimeSeconds *
                                           _wavFileInfo._samplingRate);
  ...
  // RMS計測
  rms = 0.0f;
  for (Csm::csmUint32 channelCount = 0;
       channelCount < _wavFileInfo._numberOfChannels; channelCount++) {
    for (Csm::csmUint32 sampleCount = _sampleOffset; sampleCount < goalOffset;
         sampleCount++) {
  ...
  rms = sqrt(rms /
             (_wavFileInfo._numberOfChannels * (goalOffset - _sampleOffset)));
...
void LAppWavFileHandler::Start(std::vector<char> wavData) {
  // WAVファイルのロード
  if (!LoadWavFile(wavData)) {
    return;
  }
...
Csm::csmFloat32 LAppWavFileHandler::GetRms() const { return _lastRms; }

Csm::csmBool LAppWavFileHandler::LoadWavFile(std::vector<char> wavData) {
  Csm::csmBool ret;
  ...
  // ファイルロード
  Csm::csmByte *bytes = new Csm::csmByte[wavData.size()];
  std::copy(wavData.begin(), wavData.end(), reinterpret_cast<char *>(bytes));
  _byteReader._fileByte = bytes;
  _byteReader._fileSize = wavData.size();
  ...
  // ファイル名
  _wavFileInfo._fileName = "PiperTTS";
  ...
  do {
    ...
    // 領域確保
    _pcmData = static_cast<Csm::csmFloat32 **>(
        CSM_MALLOC(sizeof(Csm::csmFloat32 *) * _wavFileInfo._numberOfChannels));
    for (Csm::csmUint32 channelCount = 0;
         channelCount < _wavFileInfo._numberOfChannels; channelCount++) {
    ...
    // 波形データ取得
    for (Csm::csmUint32 sampleCount = 0;
         sampleCount < _wavFileInfo._samplesPerChannel; sampleCount++) {
      for (Csm::csmUint32 channelCount = 0;
           channelCount < _wavFileInfo._numberOfChannels; channelCount++) {
