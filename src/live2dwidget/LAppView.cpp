...
void LAppView::Initialize() {
  int width, height;
  LAppDelegate::GetInstance()->GetSize(&width, &height);
  ...
  if (_renderTarget == SelectTarget_ModelFrameBuffer && _renderSprite) {
    ...
    for (csmUint32 i = 0; i < Live2DManager->GetModelNum(); i++) {
      ...
      float alpha =
          i < 1
              ? 1.0f
              : model->GetOpacity();  // 片方のみ不透明度を取得できるようにする
      _renderSprite->SetColor(1.0f, 1.0f, 1.0f, alpha);

      if (model) {
        _renderSprite->RenderImmidiate(
            model->GetRenderBuffer().GetColorBuffer(), uvVertex);
      }
    }
  }
}
...
void LAppView::InitializeSprite() {
  /*
     ...
     LAppDelegate::GetInstance()->GetTextureManager(); const string
     resourcesPath = LAppDelegate::GetInstance()->GetExecuteAbsolutePath() +
     ResourcesPath;
     ...
     _programId);
      ...
      LAppTextureManager::TextureInfo* gearTexture =
     textureManager->CreateTextureFromPngFile(resourcesPath + imageName);
     ...
     _programId);
      ...
      LAppTextureManager::TextureInfo* powerTexture =
     textureManager->CreateTextureFromPngFile(resourcesPath + imageName);
     ...
     static_cast<float>(height), 0, _programId);
  */
}
...
void LAppView::PreModelDraw(LAppModel &refModel) {
  ...
  if (_renderTarget !=
    ...
    if (!useTarget->IsValid()) {  // 描画ターゲット内部未作成の場合はここで作成
      ...
      LAppDelegate::GetInstance()->GetSize(&bufWidth, &bufHeight);

      if (bufWidth != 0 && bufHeight != 0) {
        // モデル描画キャンバス
        useTarget->CreateOffscreenSurface(static_cast<csmUint32>(bufWidth),
                                          static_cast<csmUint32>(bufHeight));
      }
    }
...
void LAppView::ResizeSprite() {
  /*
      LAppTextureManager* textureManager =
     LAppDelegate::GetInstance()->GetTextureManager(); if (!textureManager)
      {
          return;
      }
     ...
     textureManager->GetTextureInfoById(id); if (texInfo)
          {
              x = width * 0.5f;
              y = height * 0.5f;
