...
class LAppWavFileHandler {
 public:
   ...
   */
  void Start(std::vector<char> wavData);
 ...
 private:
   ...
   * @retval  false   読み込み失敗
   */
  Csm::csmBool LoadWavFile(std::vector<char> wavData);
  ...
  struct ByteReader {
    ...
    Csm::csmUint32 Get24LittleEndian() {
      const Csm::csmUint32 ret = (_fileByte[_readOffset + 2] << 16) |
                                 (_fileByte[_readOffset + 1] << 8) |
    ...
    Csm::csmUint32 Get32LittleEndian() {
      const Csm::csmUint32 ret = (_fileByte[_readOffset + 3] << 24) |
                                 (_fileByte[_readOffset + 2] << 16) |
                                 (_fileByte[_readOffset + 1] << 8) |
                                 _fileByte[_readOffset];
      _readOffset += 4;
      return ret;
    ...
    Csm::csmBool GetCheckSignature(const Csm::csmString &reference) {
      ...
      for (Csm::csmUint32 signatureOffset = 0; signatureOffset < 4;
           signatureOffset++) {
