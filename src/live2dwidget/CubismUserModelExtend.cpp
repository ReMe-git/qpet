...
CubismUserModelExtend::CubismUserModelExtend(
    const std::string modelDirectoryName,
    const std::string _currentModelDirectory)
    : CubismUserModel(),
      _modelJson(NULL),
...
void CubismUserModelExtend::ReleaseModelSetting() {
  // モーションの解放
  for (Csm::csmMap<Csm::csmString, Csm::ACubismMotion *>::const_iterator iter =
           _motions.Begin();
 ...
 */
Csm::CubismMotionQueueEntryHandle CubismUserModelExtend::StartMotion(
...
void CubismUserModelExtend::Draw(Csm::CubismMatrix44 &matrix) {
  ...
  GetRenderer<Csm::Rendering::CubismRenderer_OpenGLES2>()->SetMvpMatrix(
      &matrix);
...
void CubismUserModelExtend::SetupTextures() {
  for (csmInt32 modelTextureNumber = 0;
       modelTextureNumber < _modelJson->GetTextureCount();
       modelTextureNumber++) {
...
void CubismUserModelExtend::ModelOnUpdate(Live2dWidget *window) {
  int width, height;
  // ウィンドウサイズを取得
  width = window->width();
  ...
  if (MouseActionManager::GetInstance()->GetViewMatrix() != NULL) {
    projection.MultiplyByMatrix(
        MouseActionManager::GetInstance()->GetViewMatrix());
  }
