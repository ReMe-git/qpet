...
void LAppDelegate::Resize(int w, int h)
{
    ...
    _view->ResizeSprite();
    // ビューポート変更
    OpenGLExtraFunctions::GetInstance()->glViewport(0, 0, _windowWidth, _windowHeight);
}
...
{
    //バーテックスシェーダのコンパイル
    GLuint vertexShaderId = OpenGLExtraFunctions::GetInstance()->glCreateShader(GL_VERTEX_SHADER);
    const char* vertexShader =
        ...
        "    vuv = uv;"
        "}";
    OpenGLExtraFunctions::GetInstance()->glShaderSource(vertexShaderId, 1, &vertexShader, NULL);
    OpenGLExtraFunctions::GetInstance()->glCompileShader(vertexShaderId);

    //フラグメントシェーダのコンパイル
    GLuint fragmentShaderId = OpenGLExtraFunctions::GetInstance()->glCreateShader(GL_FRAGMENT_SHADER);
    const char* fragmentShader =
        ...
        "}";
    OpenGLExtraFunctions::GetInstance()->glShaderSource(fragmentShaderId, 1, &fragmentShader, NULL);
    ...
    OpenGLExtraFunctions::GetInstance()->glAttachShader(programId, vertexShaderId);
    OpenGLExtraFunctions::GetInstance()->glAttachShader(programId, fragmentShaderId);
