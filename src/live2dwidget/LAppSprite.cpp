    ...
    : _rect()
{
    ...
    // 何番目のattribute変数か
    _positionLocation = OpenGLExtraFunctions::GetInstance()->glGetAttribLocation(programId, "position");
    _uvLocation      = OpenGLExtraFunctions::GetInstance()->glGetAttribLocation(programId, "uv");
    _textureLocation = OpenGLExtraFunctions::GetInstance()->glGetUniformLocation(programId, "texture");
    _colorLocation = OpenGLExtraFunctions::GetInstance()->glGetUniformLocation(programId, "baseColor");
...
void LAppSprite::Render() const
{
    ...
    // attribute属性を有効にする
    OpenGLExtraFunctions::GetInstance()->glEnableVertexAttribArray(_positionLocation);
    ...
    // attribute属性を登録
    OpenGLExtraFunctions::GetInstance()->glVertexAttribPointer(_positionLocation, 2, GL_FLOAT, false, 0, positionVertex);
    OpenGLExtraFunctions::GetInstance()->glVertexAttribPointer(_uvLocation, 2, GL_FLOAT, false, 0, uvVertex);

    OpenGLExtraFunctions::GetInstance()->glUniform4f(_colorLocation, _spriteColor[0], _spriteColor[1], _spriteColor[2], _spriteColor[3]);
