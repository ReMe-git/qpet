...
class CubismUserModelExtend : public Csm::CubismUserModel {
 public:
  CubismUserModelExtend(
      ...
      const std::string _currentModelDirectory);  ///< コンストラクタ
   ...
   * モデルの状態や描画を更新する
   */
  void ModelOnUpdate(Live2dWidget *window);
 ...
 private:
  ...
  std::string
      _modelDirName;  ///< モデルセッティングが置かれたディレクトリの名称
  std::string
      _currentModelDirectory;  ///< モデルセッティングが置かれたディレクトリ
