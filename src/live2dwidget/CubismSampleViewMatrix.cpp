...
CubismSampleViewMatrix::CubismSampleViewMatrix(
  ...
  if (windowWidth > windowHeight) {
    ...
    // 行列の拡大率を相対的に設定
    deviceToScreen->ScaleRelative(screenW / windowWidth,
                                  -screenW / windowWidth);
  } else {
    float screenH = fabsf(top - bottom);
    // 行列の拡大率を相対的に設定
    deviceToScreen->ScaleRelative(screenH / windowHeight,
  ...
  SetMaxScreenRect(
      LAppDefine::ViewLogicalMaxLeft, LAppDefine::ViewLogicalMaxRight,
      LAppDefine::ViewLogicalMaxBottom, LAppDefine::ViewLogicalMaxTop);
}
