...
class ACubismMotion {
 public:
   ...
   */
  void UpdateParameters(CubismModel *model,
   ...
   */
  void SetupMotionQueueEntry(CubismMotionQueueEntry *motionQueueEntry,
                             csmFloat32 userTimeSeconds);
   ...
   *
   * @return
   * 登録されているモーション再生終了コールバック関数。NULLのとき、関数は何も登録されていない。
   */
   ...
   * @param[in]   onFinishedMotionCustomData  ユーザー任意データ
   */
  void SetFinishedMotionHandlerAndMotionCustomData(
   ...
   * @param[in]   userTimeSeconds     デルタ時間の積算値[秒]
   */
  csmFloat32 UpdateFadeWeight(CubismMotionQueueEntry *motionQueueEntry,
                              csmFloat32 userTimeSeconds);
 ...
 protected:
  ...
  virtual void DoUpdateParameters(CubismModel *model,
                                  csmFloat32 userTimeSeconds, csmFloat32 weight,
                                  CubismMotionQueueEntry *motionQueueEntry) = 0;
  ...
  FinishedMotionCallback
      _onFinishedMotion;  ///< モーション再生終了コールバック関数ポインタ
  void *
      _onFinishedMotionCustomData;  ///< モーション再生終了コールバックに戻されるデータ
};
