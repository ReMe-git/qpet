...
CubismMotionManager::CubismMotionManager()
    : _currentPriority(0), _reservePriority(0) {}
...
CubismMotionQueueEntryHandle CubismMotionManager::StartMotionPriority(
...
csmBool CubismMotionManager::UpdateMotion(CubismModel *model,
                                          csmFloat32 deltaTimeSeconds) {
  _userTimeSeconds += deltaTimeSeconds;

  const csmBool updated =
      CubismMotionQueueManager::DoUpdateMotion(model, _userTimeSeconds);
