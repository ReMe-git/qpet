...
const csmFloat32 CubismExpressionMotion::DefaultAdditiveValue =
    0.0f;  ///<  加算適用の初期値
const csmFloat32 CubismExpressionMotion::DefaultMultiplyValue =
    1.0f;  ///<  乗算適用の初期値
...
CubismExpressionMotion *CubismExpressionMotion::Create(const csmByte *buffer,
                                                       csmSizeInt size) {
...
void CubismExpressionMotion::DoUpdateParameters(
  ...
  for (csmUint32 i = 0; i < _parameters.GetSize(); ++i) {
    ...
    switch (parameter.BlendType) {
      case Additive: {
        model->AddParameterValue(parameter.ParameterId, parameter.Value,
                                 weight);  // 相対変化 加算
        break;
      }
      case Multiply: {
        model->MultiplyParameterValue(parameter.ParameterId, parameter.Value,
                                      weight);  // 相対変化 乗算
        break;
      }
      case Overwrite: {
        model->SetParameterValue(parameter.ParameterId, parameter.Value,
                                 weight);  // 絶対変化 上書き
        break;
      }
      default:
...
void CubismExpressionMotion::CalculateExpressionParameters(
    CubismModel *model, csmFloat32 userTimeSeconds,
    CubismMotionQueueEntry *motionQueueEntry,
    csmVector<CubismExpressionMotionManager::ExpressionParameterValue>
        *expressionParameterValues,
    csmInt32 expressionIndex, csmFloat32 fadeWeight) {
  ...
  for (csmInt32 i = 0; i < expressionParameterValues->GetSize(); ++i) {
    CubismExpressionMotionManager::ExpressionParameterValue
        expressionParameterValue = expressionParameterValues->At(i);

    if (expressionParameterValue.ParameterId == NULL) {
      continue;
    }

    const csmFloat32 currentParameterValue =
        expressionParameterValue.OverwriteValue =
            model->GetParameterValue(expressionParameterValue.ParameterId);

    csmVector<ExpressionParameter> expressionParameters =
    ...
    if (parameterIndex < 0) {
      if (expressionIndex == 0) {
        ...
        expressionParameterValues->At(i).AdditiveValue =
            CalculateValue(expressionParameterValue.AdditiveValue,
                           DefaultAdditiveValue, fadeWeight);

        expressionParameterValues->At(i).MultiplyValue =
            CalculateValue(expressionParameterValue.MultiplyValue,
                           DefaultMultiplyValue, fadeWeight);

        expressionParameterValues->At(i).OverwriteValue =
            CalculateValue(expressionParameterValue.OverwriteValue,
                           currentParameterValue, fadeWeight);
      }
      continue;
    }
...
csmVector<CubismExpressionMotion::ExpressionParameter>
...
#if _DEBUG
  CubismLogWarning(
      "GetFadeWeight() is a deprecated function. Please use "
      "CubismExpressionMotionManager.GetFadeWeight(int index).");
#endif  // _DEBUG
...
void CubismExpressionMotion::Parse(const csmByte *buffer, csmSizeInt size) {
  ...
  SetFadeInTime(
      root[ExpressionKeyFadeIn].ToFloat(DefaultFadeTime));  // フェードイン
  SetFadeOutTime(
      root[ExpressionKeyFadeOut].ToFloat(DefaultFadeTime));  // フェードアウト
  ...
  for (csmInt32 i = 0; i < parameterCount; ++i) {
    ...
    const CubismIdHandle parameterId = CubismFramework::GetIdManager()->GetId(
        param[ExpressionKeyId].GetRawString());  // パラメータID
    const csmFloat32 value =
        static_cast<csmFloat32>(param[ExpressionKeyValue].ToFloat());  // 値

    // 計算方法の設定
    ExpressionBlendType blendType;

    if (param[ExpressionKeyBlend].IsNull() ||
        param[ExpressionKeyBlend].GetString() == BlendValueAdd) {
      blendType = Additive;
...
csmFloat32 CubismExpressionMotion::CalculateValue(csmFloat32 source,
                                                  csmFloat32 destination,
                                                  csmFloat32 fadeWeight) {
