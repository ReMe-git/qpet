...
struct CubismMotionPoint {
  CubismMotionPoint() : Time(0.0f), Value(0.0f) {}
 ...
 */
typedef csmFloat32 (*csmMotionSegmentEvaluationFunction)(
    const CubismMotionPoint *points, const csmFloat32 time);
...
struct CubismMotionSegment {
   ...
   * コンストラクタ。
   */
  CubismMotionSegment() : Evaluate(NULL), BasePointIndex(0), SegmentType(0) {}
...
struct CubismMotionEvent {
  CubismMotionEvent() : FireTime(0.0f) {}
