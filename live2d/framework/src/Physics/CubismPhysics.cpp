...
csmFloat32 NormalizeParameterValue(
    csmFloat32 value, csmFloat32 parameterMinimum, csmFloat32 parameterMaximum,
    csmFloat32 parameterDefault, csmFloat32 normalizedMinimum,
    csmFloat32 normalizedMaximum, csmFloat32 normalizedDefault,
    csmInt32 isInverted) {
  csmFloat32 result = 0.0f;

  const csmFloat32 maxValue =
      CubismMath::Max(parameterMaximum, parameterMinimum);

  if (maxValue < value) {
    value = maxValue;
  }

  const csmFloat32 minValue =
      CubismMath::Min(parameterMaximum, parameterMinimum);

  if (minValue > value) {
    value = minValue;
  }

  const csmFloat32 minNormValue =
      CubismMath::Min(normalizedMinimum, normalizedMaximum);
  const csmFloat32 maxNormValue =
      CubismMath::Max(normalizedMinimum, normalizedMaximum);
...
void GetInputTranslationXFromNormalizedParameterValue(
    CubismVector2 *targetTranslation, csmFloat32 *targetAngle, csmFloat32 value,
    ...
    csmFloat32 weight) {
  targetTranslation->X +=
      NormalizeParameterValue(value, parameterMinimumValue,
                              parameterMaximumValue, parameterDefaultValue,
                              normalizationPosition->Minimum,
                              normalizationPosition->Maximum,
                              normalizationPosition->Default, isInverted) *
      weight;
}

void GetInputTranslationYFromNormalizedParameterValue(
    CubismVector2 *targetTranslation, csmFloat32 *targetAngle, csmFloat32 value,
    csmFloat32 parameterMinimumValue, csmFloat32 parameterMaximumValue,
    csmFloat32 parameterDefaultValue,
    CubismPhysicsNormalization *normalizationPosition,
...
void GetInputAngleFromNormalizedParameterValue(
    CubismVector2 *targetTranslation, csmFloat32 *targetAngle, csmFloat32 value,
    csmFloat32 parameterMinimumValue, csmFloat32 parameterMaximumValue,
    csmFloat32 parameterDefaultValue,
    CubismPhysicsNormalization *normalizationPosition,
  ...
  *targetAngle += NormalizeParameterValue(
                      ...
                      parameterDefaultValue, normalizationAngle->Minimum,
                      normalizationAngle->Maximum, normalizationAngle->Default,
                      isInverted) *
                  weight;
}

csmFloat32 GetOutputTranslationX(CubismVector2 translation,
                                 CubismPhysicsParticle *particles,
                                 csmInt32 particleIndex, csmInt32 isInverted,
...
csmFloat32 GetOutputTranslationY(CubismVector2 translation,
                                 CubismPhysicsParticle *particles,
                                 csmInt32 particleIndex, csmInt32 isInverted,
                                 CubismVector2 parentGravity) {
...
csmFloat32 GetOutputAngle(CubismVector2 translation,
                          CubismPhysicsParticle *particles,
                          csmInt32 particleIndex, csmInt32 isInverted,
                          CubismVector2 parentGravity) {
...
csmFloat32 GetOutputScaleTranslationX(CubismVector2 translationScale,
                                      csmFloat32 angleScale) {
  return translationScale.X;
}

csmFloat32 GetOutputScaleTranslationY(CubismVector2 translationScale,
                                      csmFloat32 angleScale) {
  return translationScale.Y;
}

csmFloat32 GetOutputScaleAngle(CubismVector2 translationScale,
                               csmFloat32 angleScale) {
  return angleScale;
}
...
void UpdateParticles(CubismPhysicsParticle *strand, csmInt32 strandCount,
                     CubismVector2 totalTranslation, csmFloat32 totalAngle,
                     CubismVector2 windDirection, csmFloat32 thresholdValue,
                     csmFloat32 deltaTimeSeconds, csmFloat32 airResistance) {
  csmInt32 i;
  ...
  for (i = 1; i < strandCount; ++i) {
    ...
    direction.X = ((CubismMath::CosF(radian) * direction.X) -
                   (direction.Y * CubismMath::SinF(radian)));
    direction.Y = ((CubismMath::SinF(radian) * direction.X) +
                   (direction.Y * CubismMath::CosF(radian)));
    ...
    strand[i].Position =
        strand[i - 1].Position + (newDirection * strand[i].Radius);
 ...
 * @param thresholdValue        Threshold of movement.
 */
void UpdateParticlesForStabilization(CubismPhysicsParticle *strand,
                                     csmInt32 strandCount,
                                     CubismVector2 totalTranslation,
                                     csmFloat32 totalAngle,
                                     CubismVector2 windDirection,
                                     csmFloat32 thresholdValue) {
  csmInt32 i;
  csmFloat32 totalRadian;
  CubismVector2 currentGravity;
  CubismVector2 force;
...
void UpdateOutputParameterValue(csmFloat32 *parameterValue,
                                csmFloat32 parameterValueMinimum,
                                csmFloat32 parameterValueMaximum,
                                csmFloat32 translation,
...
CubismPhysics::CubismPhysics() : _physicsRig(NULL) {
  // set default options.
...
void CubismPhysics::Initialize() {
  ...
  for (settingIndex = 0; settingIndex < _physicsRig->SubRigCount;
       ++settingIndex) {
...
void CubismPhysics::Parse(const csmByte *physicsJson, csmSizeInt size) {
  ...
  _physicsRig->Settings.UpdateSize(_physicsRig->SubRigCount,
                                   CubismPhysicsSubRig(), true);
  _physicsRig->Inputs.UpdateSize(json->GetTotalInputCount(),
                                 CubismPhysicsInput(), true);
  _physicsRig->Outputs.UpdateSize(json->GetTotalOutputCount(),
                                  CubismPhysicsOutput(), true);
  _physicsRig->Particles.UpdateSize(json->GetVertexCount(),
                                    CubismPhysicsParticle(), true);
  ...
  for (csmUint32 i = 0; i < _physicsRig->Settings.GetSize(); ++i) {
    _physicsRig->Settings[i].NormalizationPosition.Minimum =
        json->GetNormalizationPositionMinimumValue(i);
    _physicsRig->Settings[i].NormalizationPosition.Maximum =
        json->GetNormalizationPositionMaximumValue(i);
    _physicsRig->Settings[i].NormalizationPosition.Default =
        json->GetNormalizationPositionDefaultValue(i);

    _physicsRig->Settings[i].NormalizationAngle.Minimum =
        json->GetNormalizationAngleMinimumValue(i);
    _physicsRig->Settings[i].NormalizationAngle.Maximum =
        json->GetNormalizationAngleMaximumValue(i);
    _physicsRig->Settings[i].NormalizationAngle.Default =
    ...
    for (csmInt32 j = 0; j < _physicsRig->Settings[i].InputCount; ++j) {
      ...
      _physicsRig->Inputs[inputIndex + j].Source.Id =
    ...
    for (csmInt32 j = 0; j < _physicsRig->Settings[i].OutputCount; ++j) {
      ...
      _physicsRig->Outputs[outputIndex + j].VertexIndex =
          json->GetOutputVertexIndex(i, j);
      _physicsRig->Outputs[outputIndex + j].AngleScale =
          json->GetOutputAngleScale(i, j);
      _physicsRig->Outputs[outputIndex + j].Weight =
          json->GetOutputWeight(i, j);
      _physicsRig->Outputs[outputIndex + j].Destination.TargetType =
          CubismPhysicsTargetType_Parameter;

      _physicsRig->Outputs[outputIndex + j].Destination.Id =
          json->GetOutputsDestinationId(i, j);
      if (strcmp(json->GetOutputType(i, j), PhysicsTypeTagX) == 0) {
        ...
        _physicsRig->Outputs[outputIndex + j].GetScale =
      ...
      _physicsRig->Outputs[outputIndex + j].Reflect =
    ...
    for (csmInt32 j = 0; j < _physicsRig->Settings[i].ParticleCount; ++j) {
      _physicsRig->Particles[particleIndex + j].Mobility =
          json->GetParticleMobility(i, j);
      _physicsRig->Particles[particleIndex + j].Delay =
          json->GetParticleDelay(i, j);
      _physicsRig->Particles[particleIndex + j].Acceleration =
          json->GetParticleAcceleration(i, j);
      _physicsRig->Particles[particleIndex + j].Radius =
          json->GetParticleRadius(i, j);
      _physicsRig->Particles[particleIndex + j].Position =
...
void CubismPhysics::Stabilization(CubismModel *model) {
  ...
  for (settingIndex = 0; settingIndex < _physicsRig->SubRigCount;
    ...
    currentParticles =
        &_physicsRig->Particles[currentSetting->BaseParticleIndex];
    ...
    for (i = 0; i < currentSetting->InputCount; ++i) {
      ...
      currentInputs[i].GetNormalizedParameterValue(
          ...
          &currentSetting->NormalizationAngle, currentInputs[i].Reflect,
          weight);
    ...
    for (i = 0; i < currentSetting->OutputCount; ++i) {
      ...
      outputValue = currentOutputs[i].GetValue(
          translation, currentParticles, particleIndex,
          currentOutputs[i].Reflect, _options.Gravity);
...
/// the output to the parameters is interpolated with the saved previous result
/// of the pendulum calculation.
///
/// 図で示すと[1]と[2]で補間されます。
/// The figure shows the interpolation between [1] and [2].
///
/// 補間の重みは最新の振り子計算タイミングと次回のタイミングの間で見た現在時間で決定する。
/// The weight of the interpolation are determined by the current time seen
/// between the latest pendulum calculation timing and the next timing.
///
...
/// If there is no FPS information in physics3.json, it is always set in the
/// previous pendulum state.
///
/// この仕様は補間範囲を逸脱したことが原因の震えたような見た目を回避を目的にしている。
/// The purpose of this specification is to avoid the quivering appearance
/// caused by deviations from the interpolation range.
///
/// ------------ time -------------->
///
///    　　　　　　　　|+++++|------| <- weight
...
void CubismPhysics::Evaluate(CubismModel *model, csmFloat32 deltaTimeSeconds) {
  ...
  while (_currentRemainTime >= physicsDeltaTime) {
    ...
    for (settingIndex = 0; settingIndex < _physicsRig->SubRigCount;
      ...
      currentOutputs = &_physicsRig->Outputs[currentSetting->BaseOutputIndex];
      for (i = 0; i < currentSetting->OutputCount; ++i) {
        _previousRigOutputs[settingIndex].outputs[i] =
            _currentRigOutputs[settingIndex].outputs[i];
      }
    }

    // 入力キャッシュとパラメータで線形補間してUpdateParticlesするタイミングでの入力を計算する。
    // Calculate the input at the timing to UpdateParticles by linear
    // interpolation with the _parameterInputCaches and parameterValues.
    // _parameterCachesはグループ間での値の伝搬の役割があるので_parameterInputCachesとの分離が必要。
    // _parameterCaches needs to be separated from _parameterInputCaches
    ...
    for (csmInt32 j = 0; j < model->GetParameterCount(); ++j) {
      _parameterCaches[j] = _parameterInputCaches[j] * (1.0f - inputWeight) +
  ...
  for (settingIndex = 0; settingIndex < _physicsRig->SubRigCount;
    ...
    for (i = 0; i < currentSetting->OutputCount; ++i) {
      ...
      UpdateOutputParameterValue(
          ...
          parameterMinimumValues[currentOutputs[i].DestinationParameterIndex],
          parameterMaximumValues[currentOutputs[i].DestinationParameterIndex],
          _previousRigOutputs[settingIndex].outputs[i] * (1 - weight) +
              _currentRigOutputs[settingIndex].outputs[i] * weight,
          &currentOutputs[i]);
    }
  }
}

void CubismPhysics::SetOptions(const Options &options) { _options = options; }
