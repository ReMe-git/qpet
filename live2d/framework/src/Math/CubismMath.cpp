...
csmFloat32 CubismMath::DirectionToDegrees(CubismVector2 from,
                                          CubismVector2 to) {
  csmFloat32 radian;
  csmFloat32 degree;
...
csmFloat32 CubismMath::QuadraticEquation(csmFloat32 a, csmFloat32 b,
                                         csmFloat32 c) {
...
csmFloat32 CubismMath::CardanoAlgorithmForBezier(csmFloat32 a, csmFloat32 b,
                                                 csmFloat32 c, csmFloat32 d) {
  if (CubismMath::AbsF(a) < CubismMath::Epsilon) {
  ...
  if (discriminant < 0.0f) {
    ...
    csmFloat32 root2 =
        t1 * CubismMath::CosF((phi + 2.0f * CubismMath::Pi) / 3.0f) - ba / 3.0f;
    if (abs(root2 - center) < threshold) {
      return RangeF(root2, 0.0f, 1.0f);
    }

    csmFloat32 root3 =
        t1 * CubismMath::CosF((phi + 4.0f * CubismMath::Pi) / 3.0f) - ba / 3.0f;
...
csmFloat32 CubismMath::ModF(csmFloat32 dividend, csmFloat32 divisor) {
  if (!isfinite(dividend) || divisor == 0 || isnan(dividend) ||
      isnan(divisor)) {
    CubismLogWarning("dividend: %f, divisor: %f ModF() returns 'NaN'.",
                     dividend, divisor);
    return NAN;
  }
  ...
  // 絶対値で割り算する。
  csmFloat32 result =
      absDividend - floorf(absDividend / absDivisor) * absDivisor;
