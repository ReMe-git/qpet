...
class CubismViewMatrix : public CubismMatrix44 {
 public:
   ...
   */
  void SetScreenRect(csmFloat32 left, csmFloat32 right, csmFloat32 bottom,
                     csmFloat32 top);
   ...
   * @param[in]   top     上辺のY軸の位置
   */
  void SetMaxScreenRect(csmFloat32 left, csmFloat32 right, csmFloat32 bottom,
                        csmFloat32 top);
 ...
 private:
  csmFloat32
      _screenLeft;  ///< デバイスに対応する論理座標上の範囲（左辺X軸位置）
  csmFloat32
      _screenRight;  ///< デバイスに対応する論理座標上の範囲（右辺X軸位置）
  csmFloat32 _screenTop;  ///< デバイスに対応する論理座標上の範囲（下辺Y軸位置）
  csmFloat32
      _screenBottom;  ///< デバイスに対応する論理座標上の範囲（上辺Y軸位置）
