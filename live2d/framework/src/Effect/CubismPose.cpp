...
CubismPose::PartData::PartData(const PartData &v)
    : ParameterIndex(0), PartIndex(0) {
  PartId = v.PartId;

  for (csmVector<PartData>::const_iterator ite = v.Link.Begin();
       ite != v.Link.End(); ++ite) {
    Link.PushBack(*ite);
  }
}
...
CubismPose::CubismPose()
    : _fadeTimeSeconds(DefaultFadeInSeconds), _lastModel(NULL) {}
  ...
  for (csmInt32 poseIndex = 0; poseIndex < poseCount; ++poseIndex) {
    ...
    for (csmInt32 groupIndex = 0; groupIndex < idCount; ++groupIndex) {
      ...
      const CubismIdHandle parameterId =
          CubismFramework::GetIdManager()->GetId(partInfo[Id].GetRawString());
      ...
      if (!partInfo[Link].IsNull()) {
        ...
        for (csmInt32 linkIndex = 0; linkIndex < linkCount; ++linkIndex) {
          PartData linkPart;
          const CubismIdHandle linkId = CubismFramework::GetIdManager()->GetId(
              linkListInfo[linkIndex].GetString());
...
void CubismPose::Delete(CubismPose *pose) { CSM_DELETE_SELF(CubismPose, pose); }
...
void CubismPose::CopyPartOpacities(CubismModel *model) {
  for (csmUint32 groupIndex = 0; groupIndex < _partGroups.GetSize();
       ++groupIndex) {
    ...
    for (csmUint32 linkIndex = 0; linkIndex < partData.Link.GetSize();
         ++linkIndex) {
...
void CubismPose::DoFade(CubismModel *model, csmFloat32 deltaTimeSeconds,
                        csmInt32 beginIndex, csmInt32 partGroupCount) {
  ...
  for (csmInt32 i = beginIndex; i < beginIndex + partGroupCount; ++i) {
    ...
    // 非表示パーツの設定
      ...
      if (newOpacity < Phi) {
        a1 =
            newOpacity * (Phi - 1) / Phi + 1.0f;  // (0,1),(phi,phi)を通る直線式
      } else {
        a1 = (1 - newOpacity) * Phi /
             (1.0f - Phi);  // (1,0),(phi,phi)を通る直線式
      }
      ...
      if (opacity > a1) {
        opacity =
            a1;  // 計算の不透明度よりも大きければ（濃ければ）不透明度を上げる
      }
...
void CubismPose::UpdateParameters(CubismModel *model,
                                  csmFloat32 deltaTimeSeconds) {
