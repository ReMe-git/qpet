...
CubismBreath *CubismBreath::Create() { return CSM_NEW CubismBreath(); }
...
void CubismBreath::SetParameters(
    const csmVector<BreathParameterData> &breathParameters) {
  _breathParameters = breathParameters;
}

const csmVector<CubismBreath::BreathParameterData> &
CubismBreath::GetParameters() const {
  return _breathParameters;
}

void CubismBreath::UpdateParameters(CubismModel *model,
  ...
  for (csmUint32 i = 0; i < _breathParameters.GetSize(); ++i) {
    ...
    model->AddParameterValue(
        data->ParameterId, data->Offset + (data->Peak * sinf(t / data->Cycle)),
        data->Weight);
  }
}
