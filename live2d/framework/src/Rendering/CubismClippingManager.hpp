...
namespace {
const csmInt32 ColorChannelCount =
    4;  // 実験時に1チャンネルの場合は1、RGBだけの場合は3、アルファも含める場合は4
const csmInt32 ClippingMaskMaxCountOnDefault =
    36;  // 通常のフレームバッファ1枚あたりのマスク最大数
const csmInt32 ClippingMaskMaxCountOnMultiRenderTexture =
    32;  // フレームバッファが2枚以上ある場合のフレームバッファ1枚あたりのマスク最大数
}  // namespace
#endif
...
class CubismClippingManager {
 public:
   ...
   *          作っていなければNULLを返す
   *
   * @param[in]   drawableMasks    ->
   * 描画オブジェクトをマスクする描画オブジェクトのリスト
   * @param[in]   drawableMaskCounts ->
   * 描画オブジェクトをマスクする描画オブジェクトの数
   * @return
   * 該当するクリッピングマスクが存在すればインスタンスを返し、なければNULLを返す。
   */
  T_ClippingContext *FindSameClip(const csmInt32 *drawableMasks,
   ...
   */
  void createMatrixForMask(csmBool isRightHanded, csmRectF *layoutBoundsOnTex01,
                           csmFloat32 scaleX, csmFloat32 scaleY);
   ...
   */
  void CalcClippedDrawTotalBounds(CubismModel &model,
   ...
   *
   * @param[in]   channelIndex   ->   カラーチャンネル(RGBA)の番号(0:R , 1:G ,
   * 2:B, 3:A)
   */
  CubismRenderer::CubismTextureColor *GetChannelFlagAsColor(
      csmInt32 channelIndex);
 ...
 protected:
  T_OffscreenSurface
      *_currentMaskBuffer;  /// オフスクリーンサーフェイスのアドレス
  csmVector<csmBool> _clearedMaskBufferFlags;  /// マスクのクリアフラグの配列

  csmVector<CubismRenderer::CubismTextureColor *> _channelColors;
  csmVector<T_ClippingContext *>
      _clippingContextListForMask;  ///< マスク用クリッピングコンテキストのリスト
  csmVector<T_ClippingContext *>
      _clippingContextListForDraw;  ///< 描画用クリッピングコンテキストのリスト
  CubismVector2
      _clippingMaskBufferSize;  ///< クリッピングマスクのバッファサイズ（初期値:256）
