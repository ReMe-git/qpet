...
class CubismUserModel {
 public:
   ...
   * @param[in]   shouldCheckMocConsistency MOCの整合性チェックフラグ(初期値 :
   * false)
   */
  virtual void LoadModel(const csmByte *buffer, csmSizeInt size,
                         csmBool shouldCheckMocConsistency = false);
   ...
   * @return  モーションクラス
   */
  virtual ACubismMotion *LoadMotion(
   ...
   * @param[in]   name    表情の名前
   */
  virtual ACubismMotion *LoadExpression(const csmByte *buffer, csmSizeInt size,
                                        const csmChar *name);
   ...
   */
  virtual csmBool IsHit(CubismIdHandle drawableId, csmFloat32 pointX,
                        csmFloat32 pointY);
   ...
   *
   * @param[in]    caller
   * 発火したイベントを管理していたモーションマネージャー、比較用
   * @param[in]    eventValue          発火したイベントの文字列データ
