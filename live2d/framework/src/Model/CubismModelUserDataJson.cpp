...
CubismModelUserDataJson::CubismModelUserDataJson(const csmByte *buffer,
                                                 csmSizeInt size) {
  CreateCubismJson(buffer, size);
}

CubismModelUserDataJson::~CubismModelUserDataJson() { DeleteCubismJson(); }
...
csmString CubismModelUserDataJson::GetUserDataTargetType(
    const csmInt32 i) const {
...
CubismIdHandle CubismModelUserDataJson::GetUserDataId(const csmInt32 i) const {
  return CubismFramework::GetIdManager()->GetId(
      _json->GetRoot()[UserData][i][Id].GetRawString());
}

const csmChar *CubismModelUserDataJson::GetUserDataValue(
    const csmInt32 i) const {
