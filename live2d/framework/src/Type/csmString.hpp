...
class csmString {
 public:
   ...
   * @param[in]   usePtr  ->
   * trueなら引数のポインタを保持する（インスタンスと共に破棄される）<br>
   * falseなら内部でコピーを実行する
   */
 ...
 protected:
  ...
  /**
   * @brief
   * csmStringインスタンスの初期化関数。文字列のセットとハッシュコードの算出を行う。
   *
   * @param[in]   c       ->  文字列のポインタ
   * @param[in]   length  ->  文字列の長さ
   * @param[in]   usePtr  ->
   * trueなら第一引数のポインタを保持する(インスタンスと共に破棄される)。<br>
   * falseなら内部でコピーを実行する
   */
 ...
 private:
  static const csmInt32 SmallLength =
      64;  ///< この長さ-1未満の文字列は内部バッファを使用
  ...
  csmChar _small
      [SmallLength];  ///< 文字列の長さがSmallLength-1未満の場合はこちらを使用
