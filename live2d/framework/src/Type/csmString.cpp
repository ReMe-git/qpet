...
csmChar *GetEmptyString() { return const_cast<csmChar *>(s_emptyString); }
}  // namespace

csmString::csmString() : _ptr(NULL), _length(0), _hashcode(-1) {
  this->_small[0] = '\0';
...
csmString csmString::operator+(const csmString &s) const {
  ...
  if (len1 + len2 < SmallLength - 1) {
    ...
    //
    memcpy(newptr, GetRawString(),
           sizeof(csmChar) * len1);  // nullを含めない
    memcpy(&newptr[len1], s.GetRawString(),
           sizeof(csmChar) * (s._length + 1));  // nullを含めて+1
    ...
    csmChar *newptr =
        static_cast<csmChar *>(CSM_MALLOC(sizeof(csmChar) * (len1 + len2 + 1)));
...
csmString csmString::operator+(const csmChar *c) const {
  ...
  if (len1 + len2 < SmallLength - 1) {
    ...
    //
    memcpy(newptr, this->GetRawString(),
           sizeof(csmChar) * len1);  // nullを含めない
    memcpy(&newptr[len1], c,
           sizeof(csmChar) * (len2 + 1));  // nullを含めて+1
...
csmString &csmString::operator+=(const csmString &s) {
  ...
  if (this->_length + s._length < SmallLength - 1) {
    ...
    memcpy(&newptr[len1], s.GetRawString(),
           s._length + 1);  // nullを含めて+1

    Clear();  // 現在のポインタを開放してから処理する
    Initialize(newptr, len1 + s._length, false);
  } else {
    newptr = static_cast<csmChar *>(
        CSM_MALLOC(sizeof(csmChar) * (this->_length + s._length + 1)));
...
void csmString::Copy(const csmChar *c, csmInt32 length) {
  ...
  if (this->_length < SmallLength - 1) {
    ...
    this->_ptr =
        static_cast<csmChar *>(CSM_MALLOC(sizeof(csmChar) * (length + 1)));
