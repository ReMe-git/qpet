...
class Value {
 ...
 public:
  static Value *
      ErrorValue;  ///< 一時的な返り値として返すエラー。CubismFramework::Dispose()するまではCSM_DELETEしない。
  static Value *
      NullValue;  ///< 一時的な返り値として返すNULL。CubismFramework::Dispose()するまではCSM_DELETEしない。
  ...
  virtual const csmString &GetString(const csmString &defaultValue = "",
                                     const csmString &indent = "") = 0;

  /**
   * @brief   要素を文字列で返す(csmChar*)
   *
   */
  virtual const csmChar *GetRawString(const csmString &defaultValue = "",
                                      const csmString &indent = "") {
   ...
   *
   */
  virtual csmFloat32 ToFloat(csmFloat32 defaultValue = 0.0f) {
    return defaultValue;
  }
   ...
   *
   */
  virtual csmVector<Value *> *GetVector(
      csmVector<Value *> *defaultValue = NULL) {
    if (!defaultValue) {
   ...
   *
   */
  virtual csmMap<csmString, Value *> *GetMap(
      csmMap<csmString, Value *> *defaultValue = NULL) {
    return defaultValue;
  }
  ...
  virtual Value &operator[](csmInt32 index) {
    return *(
        ErrorValue->SetErrorNotForClientCall(CSM_JSON_ERROR_TYPE_MISMATCH));
  }
   ...
   * @return  マップのキー一覧
   */
  virtual csmVector<csmString> &GetKeys() { return *s_dummyKeys; }
...
class CubismJson {
 ...
 protected:
  ...
  /**
   * @brief
   * 次の「"」までの文字列をパースする。文字列は外部で解放する必要がある。
   *
   ...
   * @return      パースした文字列要素
   */
  csmString ParseString(const csmChar *string, csmInt32 length, csmInt32 begin,
                        csmInt32 *outEndPos);
   ...
   */
  Value *ParseObject(const csmChar *buffer, csmInt32 length, csmInt32 begin,
                     csmInt32 *outEndPos);
   ...
   * @return      パースから取得したValueオブジェクト
   */
  Value *ParseArray(const csmChar *buffer, csmInt32 length, csmInt32 begin,
                    csmInt32 *outEndPos);

  /**
   * @brief
   * JSONエレメントからValue(float,String,Value*,Array,null,true,false)をパースする<br>
   *           エレメントの書式に応じて内部でParseString(), ParseObject(),
   * ParseArray()を呼ぶ<br>
   *
   * @param[in]   buffer  ->  JSONエレメントのバッファ
   * @param[in]   length  ->  パースする長さ
   ...
   * @return      パースから取得したValueオブジェクト
   */
  Value *ParseValue(const csmChar *buffer, csmInt32 length, csmInt32 begin,
                    csmInt32 *outEndPos);
...
class Float : public Value {
 public:
   ...
   * @brief   コンストラクタ
   */
  Float(csmFloat32 v) : Value() { this->_value = v; }
...
class Error : public String {
 ...
 protected:
   ...
   * @brief   引数付きコンストラクタ
   */
  Error(const csmString &s, csmBool isStatic)
      : String(s), _isStatic(isStatic) {}
...
class Map : public Value {
 public:
   ...
   */
  virtual Value &operator[](const csmChar *s) {
    for (csmMap<csmString, Value *>::const_iterator iter = _map.Begin();
         iter != _map.End(); ++iter) {
   ...
   * @brief    Mapに要素を追加する
   */
  void Put(csmString &key, Value *v) { _map[key] = v; }
