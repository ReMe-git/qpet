...
CubismJson::CubismJson() : _error(NULL), _lineCount(0), _root(NULL) {}
...
Value &CubismJson::GetRoot() const { return *_root; }

csmBool CubismJson::ParseBytes(const csmByte *buffer, csmInt32 size) {
  csmInt32 endPos;
  _root =
      ParseValue(reinterpret_cast<const csmChar *>(buffer), size, 0, &endPos);
  ...
  if (_error) {
    ...
    _root = CSM_NEW Error(
        _error, false);  // rootは開放されるのでエラーオブジェクトを別途作る
    return false;
  }
  return true;
}

csmString CubismJson::ParseString(const csmChar *string, csmInt32 length,
                                  csmInt32 begin, csmInt32 *outEndPos) {
  if (_error) {
    return NULL;
  }
  ...
  for (; i < length; i++) {
    ...
    switch (c) {
      case '\"': {  // 終端の”,
        // エスケープ文字は別に処理されるのでここにはこない
        *outEndPos = i + 1;  // ”の次の文字
        ret.Append(static_cast<const csmChar *>(string + buf_start),
                   (i - buf_start));
        return ret;
      }
      case '\\': {  // エスケープの場合
...
Value *CubismJson::ParseObject(const csmChar *buffer, csmInt32 length,
                               csmInt32 begin, csmInt32 *outEndPos) {
  if (_error) {
    return NULL;
  }
...
Value *CubismJson::ParseArray(const csmChar *buffer, csmInt32 length,
                              csmInt32 begin, csmInt32 *outEndPos) {
  if (_error) {
    return NULL;
  }
...
Value *CubismJson::ParseValue(const csmChar *buffer, csmInt32 length,
                              csmInt32 begin, csmInt32 *outEndPos) {
  if (_error) {
    return NULL;
  }
  ...
  for (; i < length; i++) {
    ...
    switch (c) {
      ...
      case '\"':
        return CSM_NEW String(ParseString(buffer, length, i + 1,
                                          outEndPos));  //\"の次の文字から
      case '[':
