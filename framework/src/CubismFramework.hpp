...
void *operator new(Live2D::Cubism::Framework::csmSizeType size,
                   Live2D::Cubism::Framework::CubismAllocationTag tag,
                   const Live2D::Cubism::Framework::csmChar *fileName,
                   Live2D::Cubism::Framework::csmInt32 lineNumber);
void *operator new(Live2D::Cubism::Framework::csmSizeType size,
                   Live2D::Cubism::Framework::csmUint32 alignment,
...
template <typename T>
void CsmDelete(T *address, const Live2D::Cubism::Framework::csmChar *fileName,
               Live2D::Cubism::Framework::csmInt32 lineNumber) {
  if (!address) {
    return;
  }
  ...
  operator delete(reinterpret_cast<void *>(address),
                  Live2D::Cubism::Framework::GlobalTag, fileName, lineNumber);
}
...
#define CSM_DELETE_SELF(type, obj)                                       \
  do {                                                                   \
    ...
    obj->~type();                                                        \
    operator delete(obj, Live2D::Cubism::Framework::GlobalTag, __FILE__, \
                    __LINE__);                                           \
  } while (0)
#define CSM_DELETE(obj) CsmDelete(obj, __FILE__, __LINE__)
#define CSM_MALLOC(size) \
  Live2D::Cubism::Framework::CubismFramework::Allocate(size, __FILE__, __LINE__)
#define CSM_MALLOC_ALLIGNED(size, align)                       \
  Live2D::Cubism::Framework::CubismFramework::AllocateAligned( \
      size, align, __FILE__, __LINE__)
#define CSM_FREE(ptr)                                                   \
  Live2D::Cubism::Framework::CubismFramework::Deallocate(ptr, __FILE__, \
                                                         __LINE__)
#define CSM_FREE_ALLIGNED(ptr)                                                 \
  Live2D::Cubism::Framework::CubismFramework::DeallocateAligned(ptr, __FILE__, \
                                                                __LINE__)
...
void CsmDelete(T *address) {
  ...
  operator delete(reinterpret_cast<void *>(address),
                  Live2D::Cubism::Framework::GlobalTag);
}
...
#define CSM_MALLOC(size) \
  Live2D::Cubism::Framework::CubismFramework::Allocate(size)
#define CSM_MALLOC_ALLIGNED(size, align) \
  Live2D::Cubism::Framework::CubismFramework::AllocateAligned(size, align)
#define CSM_FREE(ptr) \
  Live2D::Cubism::Framework::CubismFramework::Deallocate(ptr)
#define CSM_FREE_ALLIGNED(ptr) \
  Live2D::Cubism::Framework::CubismFramework::DeallocateAligned(ptr)
 ...
 * @see #CleanUp()
 *
 * @note At the start of use, call CubismFramework::StartUp() before calling
 * CubismFramework::Initialize(). <br> At the end, call
 * CubismFramework::Dispose() before calling CubismFramework::CleanUp().
 */
class CubismFramework {
 public:
   ...
   * If an empty instance is set, the Framework configuration option is not
   * used.
   *
   * @return true if Cubism Framework is available; otherwise false.
   *
   * @note Make sure to execute this function before executing the API of the
   * Framework, <br> and be sure to pass the memory allocator to the
   * argument.<br> Once the preparation is complete, the process will be
   * skipped if executed again afterwards.
   */
  static csmBool StartUp(ICubismAllocator *allocator,
                         const Option *option = NULL);

  /**
   * Enables the Cubism Framework API to reuse.
   *
   * @note Clear each parameter of CubismFramework initialized by
   * StartUp().<br> Use this method when reusing Cubism Framework that has
   * been Dispose().
   */
  static void CleanUp();
  ...
  /**
   * Initializes the resources in the Cubism Framework and makes the Model
   * ready for display.
   */
   ...
   */
  static void *Allocate(csmSizeType size, const csmChar *fileName,
                        csmInt32 lineNumber);
  ...
  static void *AllocateAligned(csmSizeType size, csmUint32 alignment,
                               const csmChar *fileName, csmInt32 lineNumber);
   ...
   * @param lineNumber Number of line of source code that called
   */
  static void Deallocate(void *address, const csmChar *fileName,
                         csmInt32 lineNumber);
   ...
   * @param lineNumber Number of line of source code that called
   */
  static void DeallocateAligned(void *address, const csmChar *fileName,
                                csmInt32 lineNumber);
