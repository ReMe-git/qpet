...
class CubismClippingManager_OpenGLES2
    : public CubismClippingManager<CubismClippingContext_OpenGLES2,
 ...
 public:
  ...
  void SetupClippingContext(CubismModel &model,
                            CubismRenderer_OpenGLES2 *renderer, GLint lastFBO,
...
/**
 * @brief
 * Cubismモデルを描画する直前のOpenGLES2のステートを保持・復帰させるクラス
 *
 */
class CubismRendererProfile_OpenGLES2 {
 ...
 private:
  ...
  GLint _lastVertexAttribArrayEnabled
      [4];  ///< モデル描画直前のテクスチャユニット1
  GLboolean
      _lastScissorTest;  ///< モデル描画直前のGL_VERTEX_ATTRIB_ARRAY_ENABLEDパラメータ
  ...
  GLboolean
      _lastColorMask[4];  ///< モデル描画直前のGL_COLOR_WRITEMASKパラメータ
...
class CubismRenderer_OpenGLES2 : public CubismRenderer {
 ...
 private:
  ...
  csmVector<CubismOffscreenSurface_OpenGLES2>
