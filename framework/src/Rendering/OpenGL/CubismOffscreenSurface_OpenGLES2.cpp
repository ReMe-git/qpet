...
void CubismOffscreenSurface_OpenGLES2::BeginDraw(GLint restoreFBO)
{
    ...
    OpenGLExtraFunctions::GetInstance()->glBindFramebuffer(GL_FRAMEBUFFER, _renderTexture);
}
...
void CubismOffscreenSurface_OpenGLES2::EndDraw()
{
    ...
    // 描画対象を戻す
    OpenGLExtraFunctions::GetInstance()->glBindFramebuffer(GL_FRAMEBUFFER, _oldFBO);
}
...
csmBool CubismOffscreenSurface_OpenGLES2::CreateOffscreenSurface(csmUint32 displayBufferWidth, csmUint32 displayBufferHeight, GLuint colorBuffer)
{
    ...
    do
    {
        ...
        OpenGLExtraFunctions::GetInstance()->glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, _colorBuffer, 0);
        OpenGLExtraFunctions::GetInstance()->glBindFramebuffer(GL_FRAMEBUFFER, tmpFramebufferObject);
...
void CubismOffscreenSurface_OpenGLES2::DestroyOffscreenSurface()
{
    ...
    if (_renderTexture!=0)
    {
        OpenGLExtraFunctions::GetInstance()->glDeleteFramebuffers(1, &_renderTexture);
        _renderTexture = 0;
    }
}
