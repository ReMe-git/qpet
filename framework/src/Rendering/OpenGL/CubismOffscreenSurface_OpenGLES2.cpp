...
void CubismOffscreenSurface_OpenGLES2::BeginDraw(GLint restoreFBO) {
  ...
  OpenGLExtraFunctions::GetInstance()->glBindFramebuffer(GL_FRAMEBUFFER,
                                                         _renderTexture);
}
...
void CubismOffscreenSurface_OpenGLES2::Clear(float r, float g, float b,
                                             float a) {
  // マスクをクリアする
...
csmBool CubismOffscreenSurface_OpenGLES2::CreateOffscreenSurface(
  ...
  do {
    ...
    OpenGLExtraFunctions::GetInstance()->glGenFramebuffers(1, &ret);
    OpenGLExtraFunctions::GetInstance()->glBindFramebuffer(GL_FRAMEBUFFER, ret);
    OpenGLExtraFunctions::GetInstance()->glFramebufferTexture2D(
        GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, _colorBuffer, 0);
    OpenGLExtraFunctions::GetInstance()->glBindFramebuffer(
        GL_FRAMEBUFFER, tmpFramebufferObject);
...
void CubismOffscreenSurface_OpenGLES2::DestroyOffscreenSurface() {
  ...
  if (_renderTexture != 0) {
    OpenGLExtraFunctions::GetInstance()->glDeleteFramebuffers(1,
                                                              &_renderTexture);
    _renderTexture = 0;
  }
}
