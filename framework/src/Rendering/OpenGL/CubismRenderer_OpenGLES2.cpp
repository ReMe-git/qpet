 ...
 ********************************************************************************************************************/
void CubismClippingManager_OpenGLES2::SetupClippingContext(
    CubismModel &model, CubismRenderer_OpenGLES2 *renderer, GLint lastFBO,
  ...
  for (csmUint32 clipIndex = 0;
    ...
    // １つのクリッピングマスクに関して
    CubismClippingContext_OpenGLES2 *cc =
  ...
  for (csmUint32 clipIndex = 0;
       clipIndex < _clippingContextListForMask.GetSize(); clipIndex++) {
    // --- 実際に１つのマスクを描く ---
    CubismClippingContext_OpenGLES2 *clipContext =
        _clippingContextListForMask[clipIndex];
    ...
    CubismOffscreenSurface_OpenGLES2 *clipContextOffscreenSurface =
        renderer->GetMaskBuffer(clipContext->_bufferIndex);
    ...
    //  movePeriod' = movePeriod * scaleX + offX     [[ movePeriod' =
    //  (movePeriod - tmpBoundsOnModel.movePeriod)*scale +
...
CubismClippingContext_OpenGLES2::CubismClippingContext_OpenGLES2(
    CubismClippingManager<CubismClippingContext_OpenGLES2,
                          CubismOffscreenSurface_OpenGLES2> *manager,
    CubismModel &model, const csmInt32 *clippingDrawableIndices,
    csmInt32 clipCount)
...
CubismClippingContext_OpenGLES2::GetClippingManager() {
  return _owner;
}
 ...
 ********************************************************************************************************************/
void CubismRendererProfile_OpenGLES2::SetGlEnable(GLenum index,
                                                  GLboolean enabled) {
  if (enabled == GL_TRUE)
    glEnable(index);
  else
...
void CubismRendererProfile_OpenGLES2::SetGlEnableVertexAttribArray(
    GLuint index, GLint enabled) {
  if (enabled)
    OpenGLExtraFunctions::GetInstance()->glEnableVertexAttribArray(index);
  else
    OpenGLExtraFunctions::GetInstance()->glDisableVertexAttribArray(index);
}
...
void CubismRendererProfile_OpenGLES2::Save() {
  ...
  glActiveTexture(
      GL_TEXTURE1);  // テクスチャユニット1をアクティブに（以後の設定対象とする）
  glGetIntegerv(GL_TEXTURE_BINDING_2D, &_lastTexture1Binding2D);

  glActiveTexture(
      GL_TEXTURE0);  // テクスチャユニット0をアクティブに（以後の設定対象とする）
  glGetIntegerv(GL_TEXTURE_BINDING_2D, &_lastTexture0Binding2D);

  OpenGLExtraFunctions::GetInstance()->glGetVertexAttribiv(
      0, GL_VERTEX_ATTRIB_ARRAY_ENABLED, &_lastVertexAttribArrayEnabled[0]);
  OpenGLExtraFunctions::GetInstance()->glGetVertexAttribiv(
      1, GL_VERTEX_ATTRIB_ARRAY_ENABLED, &_lastVertexAttribArrayEnabled[1]);
  OpenGLExtraFunctions::GetInstance()->glGetVertexAttribiv(
      2, GL_VERTEX_ATTRIB_ARRAY_ENABLED, &_lastVertexAttribArrayEnabled[2]);
  OpenGLExtraFunctions::GetInstance()->glGetVertexAttribiv(
      3, GL_VERTEX_ATTRIB_ARRAY_ENABLED, &_lastVertexAttribArrayEnabled[3]);
...
void CubismRendererProfile_OpenGLES2::Restore() {
  OpenGLExtraFunctions::GetInstance()->glUseProgram(_lastProgram);
  ...
  glColorMask(_lastColorMask[0], _lastColorMask[1], _lastColorMask[2],
              _lastColorMask[3]);

  OpenGLExtraFunctions::GetInstance()->glBindBuffer(
      GL_ARRAY_BUFFER,
      _lastArrayBufferBinding);  // 前にバッファがバインドされていたら破棄する必要がある
  OpenGLExtraFunctions::GetInstance()->glBindBuffer(
      GL_ELEMENT_ARRAY_BUFFER, _lastElementArrayBufferBinding);
  ...
  OpenGLExtraFunctions::GetInstance()->glBlendFuncSeparate(
      _lastBlending[0], _lastBlending[1], _lastBlending[2], _lastBlending[3]);
}
...
void CubismRenderer_OpenGLES2::SetExtShaderMode(csmBool extMode,
                                                csmBool extPAMode) {
...
csmBool s_isInitializeGlFunctionsSuccess =
    false;  ///< 初期化が完了したかどうか。trueなら初期化完了
csmBool s_isFirstInitializeGlFunctions =
    true;  ///< 最初の初期化実行かどうか。trueなら最初の初期化実行
}  // namespace
...
void CubismRenderer_OpenGLES2::InitializeGlFunctions() {
  ...
  glActiveTexture =
      (PFNGLACTIVETEXTUREPROC)WinGlGetProcAddress("glActiveTexture");
  ...
  glGetAttribLocation =
      (PFNGLGETATTRIBLOCATIONPROC)WinGlGetProcAddress("glGetAttribLocation");
  glGetUniformLocation =
      (PFNGLGETUNIFORMLOCATIONPROC)WinGlGetProcAddress("glGetUniformLocation");
  glBlendFuncSeparate =
      (PFNGLBLENDFUNCSEPARATEPROC)WinGlGetProcAddress("glBlendFuncSeparate");
  glEnableVertexAttribArray =
      (PFNGLENABLEVERTEXATTRIBARRAYPROC)WinGlGetProcAddress(
          "glEnableVertexAttribArray");
  glDisableVertexAttribArray =
      (PFNGLDISABLEVERTEXATTRIBARRAYPROC)WinGlGetProcAddress(
          "glDisableVertexAttribArray");
  glVertexAttribPointer = (PFNGLVERTEXATTRIBPOINTERPROC)WinGlGetProcAddress(
      "glVertexAttribPointer");

  glUniformMatrix4fv =
      (PFNGLUNIFORMMATRIX4FVPROC)WinGlGetProcAddress("glUniformMatrix4fv");
  ...
  glValidateProgram =
      (PFNGLVALIDATEPROGRAMPROC)WinGlGetProcAddress("glValidateProgram");

  glGetProgramInfoLog =
      (PFNGLGETPROGRAMINFOLOGPROC)WinGlGetProcAddress("glGetProgramInfoLog");
  glCreateProgram =
      (PFNGLCREATEPROGRAMPROC)WinGlGetProcAddress("glCreateProgram");

  glDeleteProgram =
      (PFNGLDELETEPROGRAMPROC)WinGlGetProcAddress("glDeleteProgram");
  ...
  glCompileShader =
      (PFNGLCOMPILESHADERPROC)WinGlGetProcAddress("glCompileShader");
  ...
  glGenFramebuffers =
      (PFNGLGENFRAMEBUFFERSEXTPROC)WinGlGetProcAddress("glGenFramebuffers");
  glGenRenderbuffers =
      (PFNGLGENRENDERBUFFERSEXTPROC)WinGlGetProcAddress("glGenRenderbuffers");
  glBindFramebuffer =
      (PFNGLBINDFRAMEBUFFEREXTPROC)WinGlGetProcAddress("glBindFramebuffer");
  glFramebufferTexture2D =
      (PFNGLFRAMEBUFFERTEXTURE2DEXTPROC)WinGlGetProcAddress(
          "glFramebufferTexture2D");
  glDeleteFramebuffers = (PFNGLDELETEFRAMEBUFFERSEXTPROC)WinGlGetProcAddress(
      "glDeleteFramebuffers");
  glBindRenderbuffer =
      (PFNGLBINDRENDERBUFFEREXTPROC)WinGlGetProcAddress("glBindRenderbuffer");
  glDeleteRenderbuffers = (PFNGLDELETERENDERBUFFERSEXTPROC)WinGlGetProcAddress(
      "glDeleteRenderbuffers");

  glRenderbufferStorage = (PFNGLRENDERBUFFERSTORAGEEXTPROC)WinGlGetProcAddress(
      "glRenderbufferStorage");
  glFramebufferRenderbuffer =
      (PFNGLFRAMEBUFFERRENDERBUFFEREXTPROC)WinGlGetProcAddress(
          "glFramebufferRenderbuffer");
  glCheckFramebufferStatus =
      (PFNGLCHECKFRAMEBUFFERSTATUSEXTPROC)WinGlGetProcAddress(
          "glCheckFramebufferStatus");
  glGetVertexAttribiv =
      (PFNGLGETVERTEXATTRIBIVPROC)WinGlGetProcAddress("glGetVertexAttribiv");
}
...
void CubismRenderer_OpenGLES2::DoStaticRelease() {
  ...
  s_isInitializeGlFunctionsSuccess =
      false;  ///< 初期化が完了したかどうか。trueなら初期化完了
  s_isFirstInitializeGlFunctions =
      true;  ///< 最初の初期化実行かどうか。trueなら最初の初期化実行
#endif
  CubismShader_OpenGLES2::DeleteInstance();
}
...
void CubismRenderer_OpenGLES2::Initialize(CubismModel *model,
  ...
  if (model->IsUsingMasking()) {
    ...
    CubismClippingManager_OpenGLES2();  // クリッピングマスク・バッファ前処理方式を初期化
...
void CubismRenderer_OpenGLES2::PreDraw() {
  ...
  OpenGLExtraFunctions::GetInstance()->glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, 0);
  OpenGLExtraFunctions::GetInstance()->glBindBuffer(
      GL_ARRAY_BUFFER,
      0);  // 前にバッファがバインドされていたら破棄する必要がある
...
void CubismRenderer_OpenGLES2::DoDrawModel() {
  ...
  if (_clippingManager != NULL) {
    ...
    for (csmInt32 i = 0; i < _clippingManager->GetRenderTextureCount(); ++i) {
      if (_offscreenSurfaces[i].GetBufferWidth() !=
              static_cast<csmUint32>(
                  _clippingManager->GetClippingMaskBufferSize().X) ||
          _offscreenSurfaces[i].GetBufferHeight() !=
              static_cast<csmUint32>(
                  _clippingManager->GetClippingMaskBufferSize().Y)) {
  ...
  for (csmInt32 i = 0; i < drawableCount; ++i) {
        ...
        IsUsingHighPrecisionMask())  // マスクを書く必要がある
    {
      ...
      {
        // 生成したOffscreenSurfaceと同じサイズでビューポートを設定
        glViewport(0, 0, _clippingManager->GetClippingMaskBufferSize().X,
                   _clippingManager->GetClippingMaskBufferSize().Y);
        ...
        // ---------- マスク描画処理 ----------
        // マスク用RenderTextureをactiveにセット
        GetMaskBuffer(clipContext->_bufferIndex)
            ->BeginDraw(_rendererProfile._lastFBO);
...

      {
        ...
        glViewport(_rendererProfile._lastViewport[0],
                   _rendererProfile._lastViewport[1],
                   _rendererProfile._lastViewport[2],
                   _rendererProfile._lastViewport[3]);
...
void CubismRenderer_OpenGLES2::DrawMeshOpenGL(const CubismModel &model,
  ...
  glFrontFace(
      GL_CCW);  // Cubism SDK OpenGLはマスク・アートメッシュ共にCCWが表面

  if (IsGeneratingMask())  // マスク生成時
  {
    CubismShader_OpenGLES2::GetInstance()->SetupShaderProgramForMask(
        this, model, index);
  } else {
    CubismShader_OpenGLES2::GetInstance()->SetupShaderProgramForDraw(
        this, model, index);
  }
  ...
  // ポリゴンメッシュを描画する
  {
    ...
    csmUint16 *indexArray =
        const_cast<csmUint16 *>(model.GetDrawableVertexIndices(index));
    glDrawElements(GL_TRIANGLES, indexCount, GL_UNSIGNED_SHORT, indexArray);
  }

  // 後処理
  OpenGLExtraFunctions::GetInstance()->glUseProgram(0);
  SetClippingContextBufferForDraw(NULL);
  SetClippingContextBufferForMask(NULL);
}

void CubismRenderer_OpenGLES2::SaveProfile() { _rendererProfile.Save(); }

void CubismRenderer_OpenGLES2::RestoreProfile() { _rendererProfile.Restore(); }

void CubismRenderer_OpenGLES2::BindTexture(csmUint32 modelTextureIndex,
                                           GLuint glTextureIndex) {
  _textures[modelTextureIndex] = glTextureIndex;
}

const csmMap<csmInt32, GLuint> &CubismRenderer_OpenGLES2::GetBindedTextures()
    const {
  return _textures;
}

void CubismRenderer_OpenGLES2::SetClippingMaskBufferSize(csmFloat32 width,
                                                         csmFloat32 height) {
  if (_clippingManager == NULL) {
    return;
  }
...
CubismOffscreenSurface_OpenGLES2 *CubismRenderer_OpenGLES2::GetMaskBuffer(
    csmInt32 index) {
  return &_offscreenSurfaces[index];
}

void CubismRenderer_OpenGLES2::SetClippingContextBufferForMask(
    CubismClippingContext_OpenGLES2 *clip) {
  _clippingContextBufferForMask = clip;
}

CubismClippingContext_OpenGLES2 *
CubismRenderer_OpenGLES2::GetClippingContextBufferForMask() const {
  return _clippingContextBufferForMask;
}

void CubismRenderer_OpenGLES2::SetClippingContextBufferForDraw(
    CubismClippingContext_OpenGLES2 *clip) {
  _clippingContextBufferForDraw = clip;
}

CubismClippingContext_OpenGLES2 *
CubismRenderer_OpenGLES2::GetClippingContextBufferForDraw() const {
