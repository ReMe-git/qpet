...
{
    if (enabled) OpenGLExtraFunctions::GetInstance()->glEnableVertexAttribArray(index);
    else OpenGLExtraFunctions::GetInstance()->glDisableVertexAttribArray(index);
}
...
void CubismRendererProfile_OpenGLES2::Save()
{
    ...
    OpenGLExtraFunctions::GetInstance()->glGetVertexAttribiv(0, GL_VERTEX_ATTRIB_ARRAY_ENABLED, &_lastVertexAttribArrayEnabled[0]);
    OpenGLExtraFunctions::GetInstance()->glGetVertexAttribiv(1, GL_VERTEX_ATTRIB_ARRAY_ENABLED, &_lastVertexAttribArrayEnabled[1]);
    OpenGLExtraFunctions::GetInstance()->glGetVertexAttribiv(2, GL_VERTEX_ATTRIB_ARRAY_ENABLED, &_lastVertexAttribArrayEnabled[2]);
    OpenGLExtraFunctions::GetInstance()->glGetVertexAttribiv(3, GL_VERTEX_ATTRIB_ARRAY_ENABLED, &_lastVertexAttribArrayEnabled[3]);
...
void CubismRendererProfile_OpenGLES2::Restore()
{
    ...
    OpenGLExtraFunctions::GetInstance()->glBindBuffer(GL_ARRAY_BUFFER, _lastArrayBufferBinding); //前にバッファがバインドされていたら破棄する必要がある
    OpenGLExtraFunctions::GetInstance()->glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, _lastElementArrayBufferBinding);
    ...
    // restore blending
    OpenGLExtraFunctions::GetInstance()->glBlendFuncSeparate(_lastBlending[0], _lastBlending[1], _lastBlending[2], _lastBlending[3]);
}
...
void CubismRenderer_OpenGLES2::PreDraw()
{
    ...
    OpenGLExtraFunctions::GetInstance()->glBindBuffer(GL_ARRAY_BUFFER, 0); //前にバッファがバインドされていたら破棄する必要がある
