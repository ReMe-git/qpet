...
class CubismShader_OpenGLES2 {
 public:
   ...
   */
  void SetupShaderProgramForDraw(CubismRenderer_OpenGLES2 *renderer,
   ...
   * @param[in]   index                 ->  描画対象のメッシュのインデックス
   */
  void SetupShaderProgramForMask(CubismRenderer_OpenGLES2 *renderer,
                                 const CubismModel &model,
                                 const csmInt32 index);
 ...
 private:
  ...
  struct CubismShaderSet {
    ...
    GLuint
        AttributePositionLocation;  ///< シェーダプログラムに渡す変数のアドレス(Position)
    GLuint
        AttributeTexCoordLocation;  ///< シェーダプログラムに渡す変数のアドレス(TexCoord)
    GLint
        UniformMatrixLocation;  ///< シェーダプログラムに渡す変数のアドレス(Matrix)
    GLint
        UniformClipMatrixLocation;  ///< シェーダプログラムに渡す変数のアドレス(ClipMatrix)
    GLint
        SamplerTexture0Location;  ///< シェーダプログラムに渡す変数のアドレス(Texture0)
    GLint
        SamplerTexture1Location;  ///< シェーダプログラムに渡す変数のアドレス(Texture1)
    GLint
        UniformBaseColorLocation;  ///< シェーダプログラムに渡す変数のアドレス(BaseColor)
    GLint
        UniformMultiplyColorLocation;  ///< シェーダプログラムに渡す変数のアドレス(MultiplyColor)
    GLint
        UniformScreenColorLocation;  ///< シェーダプログラムに渡す変数のアドレス(ScreenColor)
    GLint
        UnifromChannelFlagLocation;  ///< シェーダプログラムに渡す変数のアドレス(ChannelFlag)
  };
   ...
   */
  GLuint LoadShaderProgram(const csmChar *vertShaderSrc,
   ...
   */
  csmBool CompileShaderSource(GLuint *outShader, GLenum shaderType,
                              const csmChar *shaderSource);
   ...
   */
  void SetVertexAttributes(const CubismModel &model, const csmInt32 index,
   ...
   * @param[in]   shaderSet             ->  シェーダープログラムのセット
   */
  void SetupTexture(CubismRenderer_OpenGLES2 *renderer,
   ...
   */
  void SetColorChannelUniformVariables(
      CubismShaderSet *shaderSet,
 ...
 private:
  ...
  csmVector<CubismShaderSet *>
      _shaderSets;  ///< ロードしたシェーダプログラムを保持する変数
};
