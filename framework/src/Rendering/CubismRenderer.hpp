...
class CubismRenderer {
 public:
  ...
  struct CubismTextureColor {
     ...
     * @brief   コンストラクタ
     */
    CubismTextureColor() : R(1.0f), G(1.0f), B(1.0f), A(1.0f){};
   ...
   *
   */
  void SetModelColor(csmFloat32 red, csmFloat32 green, csmFloat32 blue,
                     csmFloat32 alpha);
 ...
 private:
  ...
  csmBool
      _useHighPrecisionMask;  ///< falseの場合、マスクを纏めて描画する
                              ///< trueの場合、マスクはパーツ描画ごとに書き直す
};
...
class CubismClippingContext {
 public:
   ...
   *
   */
  CubismClippingContext(const csmInt32 *clippingDrawableIndices,
                        csmInt32 clipCount);
  ...
  csmInt32
      _layoutChannelIndex;  ///< RGBAのいずれのチャンネルにこのクリップを配置するか(0:R
                            ///< , 1:G , 2:B , 3:A)
  csmRectF *
      _layoutBounds;  ///< マスク用チャンネルのどの領域にマスクを入れるか(View座標-1..1,
                      ///< UVは0..1に直す)
  csmRectF *
      _allClippedDrawRect;  ///< このクリッピングで、クリッピングされる全ての描画オブジェクトの囲み矩形（毎回更新）
