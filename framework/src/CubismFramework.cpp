...
inline ICubismAllocator *GetAllocator() { return s_allocator; }
...
csmBool CubismFramework::StartUp(ICubismAllocator *allocator,
                                 const Option *option) {
  if (s_isStarted) {
  ...
  if (allocator == NULL) {
    CubismLogWarning(
        "CubismFramework::StartUp() failed, need allocator instance.");
    s_isStarted = false;
  } else {
  ...
  if (s_isStarted) {
    ...
    const csmUint32 major =
        static_cast<csmUint32>((version & 0xFF000000) >> 24);
    const csmUint32 minor =
        static_cast<csmUint32>((version & 0x00FF0000) >> 16);
    const csmUint32 patch = static_cast<csmUint32>((version & 0x0000FFFF));
    const csmUint32 versionNumber = version;

    CubismLogInfo("Live2D Cubism Core version: %02d.%02d.%04d (%d)", major,
                  minor, patch, versionNumber);
  }
...
csmBool CubismFramework::IsStarted() { return s_isStarted; }
...
csmBool CubismFramework::IsInitialized() { return s_isInitialized; }
...
CubismIdManager *CubismFramework::GetIdManager() { return s_cubismIdManager; }

#ifdef CSM_DEBUG_MEMORY_LEAKING

void *CubismFramework::Allocate(csmSizeType size, const csmChar *fileName,
                                csmInt32 lineNumber) {
  void *address = GetAllocator()->Allocate(size);

  CubismLogVerbose("CubismFramework::Allocate(0x%p, %dbytes) %s(%d)", address,
...
void *CubismFramework::AllocateAligned(csmSizeType size, csmUint32 alignment,
  ...
  CubismLogVerbose(
      "CubismFramework::AllocateAligned(0x%p, a:%d, %dbytes) %s(%d)", address,
      alignment, size, fileName, lineNumber);
...
void CubismFramework::Deallocate(void *address, const csmChar *fileName,
                                 csmInt32 lineNumber) {
  if (!address) {
    return;
  }

  CubismLogVerbose("CubismFramework::Deallocate(0x%p) %s(%d)", address,
                   fileName, lineNumber);

  if (s_allocationList) {
    for (std::vector<void *>::iterator iter = s_allocationList->begin();
         iter != s_allocationList->end(); ++iter) {
      if (*iter != address) {
        continue;
      }
...
void CubismFramework::DeallocateAligned(void *address, const csmChar *fileName,
                                        csmInt32 lineNumber) {
  if (!address) {
    return;
  }

  CubismLogVerbose("CubismFramework::DeallocateAligned(0x%p) %s(%d)", address,
                   fileName, lineNumber);
...
void *operator new(Live2D::Cubism::Framework::csmSizeType size,
  ...
  return Live2D::Cubism::Framework::CubismFramework::Allocate(size, fileName,
                                                              lineNumber);
}
...
void *operator new(Live2D::Cubism::Framework::csmSizeType size,
                   ...
                   const Live2D::Cubism::Framework::csmChar *fileName,
                   Live2D::Cubism::Framework::csmInt32 lineNumber) {
  return Live2D::Cubism::Framework::CubismFramework::AllocateAligned(
...
void operator delete(void *address,
                     ...
                     const Live2D::Cubism::Framework::csmChar *fileName,
                     Live2D::Cubism::Framework::csmInt32 lineNumber) {
  Live2D::Cubism::Framework::CubismFramework::Deallocate(address, fileName,
                                                         lineNumber);
}
...
void operator delete(void *address,
                     ...
                     const Live2D::Cubism::Framework::csmChar *fileName,
                     Live2D::Cubism::Framework::csmInt32 lineNumber) {
  Live2D::Cubism::Framework::CubismFramework::DeallocateAligned(
      address, fileName, lineNumber);
}
...
void *operator new(Live2D::Cubism::Framework::csmSizeType size,
                   Live2D::Cubism::Framework::csmUint32 alignment,
                   Live2D::Cubism::Framework::CubismAllocationAlignedTag tag) {
  return Live2D::Cubism::Framework::CubismFramework::AllocateAligned(size,
                                                                     alignment);
}
...
void operator delete(
    void *address, Live2D::Cubism::Framework::CubismAllocationAlignedTag tag) {
