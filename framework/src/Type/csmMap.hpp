...
/**
 *@brief    マップ型<br>
 *           コンシューマゲーム機等でSTLの組み込みを避けるための実装。std::map
 *の簡易版
 */
template <class _KeyT, class _ValT>
class csmMap {
 public:
   ...
   * @param[in]   fitToSize   ->
   * trueなら指定したサイズに合わせる。falseならサイズを2倍確保しておく。
   */
  ...
  class const_iterator {
   ...
   public:
     ...
     * @param[in]   idx ->  コンテナから参照するインデックス値
     */
    const_iterator(const csmMap<_KeyT, _ValT> *v, csmInt32 idx)
        : _index(idx), _map(v) {}
  ...
  const iterator Erase(const iterator &ite) {
    ...
    if (index < _size - 1)
      memmove(&(_keyValues[index]), &(_keyValues[index + 1]),
              sizeof(csmPair<_KeyT, _ValT>) * (_size - index - 1));
    --_size;
 ...
 private:
  ...
  _ValT *
      _dummyValuePtr;  ///< 空の値を返すためのダミー(staticのtemplteを回避するためメンバとする）
...
csmMap<_KeyT, _ValT>::csmMap()
    : _keyValues(NULL), _dummyValuePtr(NULL), _size(0), _capacity(0) {}
...
csmMap<_KeyT, _ValT>::csmMap(csmInt32 size) : _dummyValuePtr(NULL) {
  if (size < 1) {
    ...
    _keyValues = static_cast<csmPair<_KeyT, _ValT> *>(
        CSM_MALLOC(size * sizeof(csmPair<_KeyT, _ValT>)));
...
template <class _KeyT, class _ValT>
void csmMap<_KeyT, _ValT>::PrepareCapacity(csmInt32 newSize,
                                           csmBool fitToSize) {
  if (newSize > _capacity) {
    if (_capacity == 0) {
      ...
      _keyValues = static_cast<csmPair<_KeyT, _ValT> *>(
          CSM_MALLOC(sizeof(csmPair<_KeyT, _ValT>) * newSize));
      ...
      if (!fitToSize && newSize < _capacity * 2)
        newSize = _capacity *
                  2;  // 指定サイズに合わせる必要がない場合は、２倍に広げる

      csmInt32 tmp_capacity = newSize;
      csmPair<_KeyT, _ValT> *tmp = static_cast<csmPair<_KeyT, _ValT> *>(
          CSM_MALLOC(sizeof(csmPair<_KeyT, _ValT>) * tmp_capacity));

      CSM_ASSERT(tmp != NULL);

      // 通常のMALLOCになったためコピーする
      memcpy(static_cast<void *>(tmp), static_cast<void *>(_keyValues),
             sizeof(csmPair<_KeyT, _ValT>) * _capacity);
      CSM_FREE(_keyValues);
