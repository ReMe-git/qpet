 ...
 *           コンシューマゲーム機等でSTLの組み込みを避けるための実装。std::vector
 * の簡易版
 *
 */
template <class T>
class csmVector {
 public:
   ...
   * 初期化後のキャパシティ。データサイズは_capacity * sizeof(T)
   ...
   *
   */
  T &operator[](csmInt32 index) { return _ptr[index]; }

  /**
   * @brief   []演算子のオーバーロード(const)
   *
   */
  const T &operator[](csmInt32 index) const { return _ptr[index]; }

  /**
   * @brief   インデックスで指定した要素を返す
   *
   */
  T &At(int index) { return _ptr[index]; }

  /**
   * @brief   PushBack処理.コンテナに新たな要素を追加する。
   *
   * @param[in]   value            -> PushBack処理で追加する値。
   * @param[in]   callPlacementNew ->
   * PushBack時に配置newを呼び出す場合はtrue（クラスインスタンス）
   ...
   *
   */
  void Resize(csmInt32 size, T value = T()) { UpdateSize(size, value, true); }
   ...
   * @param[in]   callPlacementNew    ->
   * UpdateSize時に配置newを呼び出す場合はtrue（クラスインスタンス）<br>
   *                                       UpdateSize時に値を単純に代入する場合はfalse(プリミティブ、ポインタ）
   *
   */
  void UpdateSize(csmInt32 size, T value = T(),
                  csmBool callPlacementNew = true);

  /**
   * @brief   コンテナのキャパシティを確保する<br>
   *
   * @param[in]   newSize ->
   * 新たなキャパシティ。引数の値が現在のサイズ未満の場合は何もしない。
   */
   ...
   * @param[in]   callPlacementNew    ->
   * Assign時に配置newを呼び出す場合はtrue（クラスインスタンス）<br>
  ...
  csmBool Remove(csmInt32 index) {
    ...
    if (index < _size - 1)
      memmove(&(_ptr[index]), &(_ptr[index + 1]),
  ...
  class iterator {
   ...
   public:
     ...
     *
     */
    iterator(csmVector<T> *v) : _index(0), _vector(v) {}
     ...
     */
    iterator(csmVector<T> *v, csmInt32 idx) : _index(idx), _vector(v) {}
     ...
     *
     */
    T &operator*() const { return this->_vector->_ptr[this->_index]; }
  ...
  class const_iterator {
   ...
   public:
     ...
     *
     */
    const_iterator(const csmVector<T> *v) : _index(0), _vector(v) {}
     ...
     * @param[in]   idx ->  コンテナから参照するインデックス値
     */
    const_iterator(const csmVector<T> *v, csmInt32 idx)
        : _index(idx), _vector(v) {}
    /**
     * @brief   =演算子のオーバーロード
     *
     */
    ...
    const_iterator operator++(csmInt32) {
      const_iterator iteold(this->_vector,
                            this->_index++);  // 古い値を保存
      return iteold;                          // 古い値を返す
    }
   ...
   */
  csmVector(const csmVector &c) { Copy(c); }
 ...
 private:
  static const csmInt32 s_defaultSize =
      10;  ///< コンテナ初期化のデフォルトサイズ
...
template <class T>
csmVector<T>::csmVector() : _ptr(NULL), _size(0), _capacity(0) {}
...
csmVector<T>::csmVector(csmInt32 initialCapacity, csmBool zeroClear) {
  if (initialCapacity < 1) {
    ...
    _ptr = static_cast<T *>(CSM_MALLOC(
        sizeof(T) * initialCapacity));  // ここだけ calloc
                                        // により、確保したバイトを0で埋める
...
template <class T>
void csmVector<T>::UpdateSize(csmInt32 newSize, T value,
                              csmBool callPlacementNew) {
...
void csmVector<T>::Assign(csmInt32 newSize, T value, csmBool callPlacementNew) {
  ...
  if (callPlacementNew) {
    for (csmInt32 i = 0; i < newSize; i++) {
      CSM_PLACEMENT_NEW(&_ptr[i])
      T(value);  // placement new 指定のアドレスに、実体を生成する
    }
  } else {
    for (csmInt32 i = 0; i < newSize; i++) {
      _ptr[i] = value;
...
template <class T>
void csmVector<T>::Insert(iterator position, iterator begin, iterator end,
                          csmBool callPlacementNew) {
  ...
  if (_size - dst_si > 0) {
    memmove(&(_ptr[dst_si + addcount]), &(_ptr[dst_si]),
            sizeof(T) * (_size - dst_si));
  }
