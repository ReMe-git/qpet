 ...
 */
typedef void (*CubismMotionEventFunction)(
...
extern const CubismMotionQueueEntryHandle
    InvalidMotionQueueEntryHandleValue;  ///< 無効なモーションの識別番号の定義
 ...
 *
 * @note 再生中に別のモーションが
 * StartMotion()された場合は、新しいモーションに滑らかに変化し旧モーションは中断する。
...
class CubismMotionQueueManager {
 public:
   ...
   */
  CubismMotionQueueEntryHandle StartMotion(ACubismMotion *motion,
                                           csmBool autoDelete);

  /**
   * @deprecated
   * 第3引数userTimeSecondsを関数内で使用していないため非推奨。StartMotion(ACubismMotion*
   * motion, csmBool autoDelete)を使用してください。
   * @brief 指定したモーションの開始
   *
   * 指定したモーションを開始する。同じタイプのモーションが既にある場合は、既存のモーションに終了フラグを立て、フェードアウトを開始させる。
   *
   ...
   */
  CubismMotionQueueEntry *GetCubismMotionQueueEntry(
   ...
   */
  void SetEventCallback(CubismMotionEventFunction callback,
 ...
 protected:
   ...
   */
  virtual csmBool DoUpdateMotion(CubismModel *model,
                                 csmFloat32 userTimeSeconds);
