...
class CubismMotion : public ACubismMotion {
 public:
   ...
   * @param[in]   onFinishedMotionHandler
   * モーション再生終了時に呼び出されるコールバック関数。NULLの場合、呼び出されない。
   * @return  作成されたインスタンス
   */
  static CubismMotion *Create(
      const csmByte *buffer, csmSizeInt size,
      FinishedMotionCallback onFinishedMotionHandler = NULL);
   ...
   * @param[in]   motionQueueEntry
   * CubismMotionQueueManagerで管理されているモーション
   */
  virtual void DoUpdateParameters(CubismModel *model,
   ...
   *
   * @param[in]   eyeBlinkParameterIds
   * 自動まばたきがかかっているパラメータIDのリスト
   * @param[in]   lipSyncParameterIds
   * リップシンクがかかっているパラメータIDのリスト
   */
  void SetEffectIds(const csmVector<CubismIdHandle> &eyeBlinkParameterIds,
   ...
   */
  virtual const csmVector<const csmString *> &GetFiredEvent(
 ...
 private:
  ...
  csmFloat32
      _sourceFrameRate;  ///< ロードしたファイルのFPS。記述が無ければデフォルト値15fpsとなる
  csmFloat32
      _loopDurationSeconds;  ///< mtnファイルで定義される一連のモーションの長さ
  csmBool _isLoop;  ///< ループするか?
  csmBool
      _isLoopFadeIn;  ///< ループ時にフェードインが有効かどうかのフラグ。初期値では有効。
  csmFloat32 _lastWeight;  ///< 最後に設定された重み

  CubismMotionData *_motionData;  ///< 実際のモーションデータ本体

  csmVector<CubismIdHandle>
      _eyeBlinkParameterIds;  ///< 自動まばたきを適用するパラメータIDハンドルのリスト。
                              ///< モデル（モデルセッティング）とパラメータを対応付ける。
  csmVector<CubismIdHandle>
      _lipSyncParameterIds;  ///< リップシンクを適用するパラメータIDハンドルのリスト。
                             ///< モデル（モデルセッティング）とパラメータを対応付ける。
  ...
  CubismIdHandle
      _modelCurveIdEyeBlink;  ///< モデルが持つ自動まばたき用パラメータIDのハンドル。
                              ///< モデルとモーションを対応付ける。
  CubismIdHandle
      _modelCurveIdLipSync;  ///< モデルが持つリップシンク用パラメータIDのハンドル。
                             ///< モデルとモーションを対応付ける。
  CubismIdHandle
      _modelCurveIdOpacity;  ///< モデルが持つ不透明度用パラメータIDのハンドル。
                             ///< モデルとモーションを対応付ける。
