...
ACubismMotion::~ACubismMotion() { this->_weight = 0.0f; }

void ACubismMotion::UpdateParameters(CubismModel *model,
...
void ACubismMotion::SetupMotionQueueEntry(
    CubismMotionQueueEntry *motionQueueEntry, csmFloat32 userTimeSeconds) {
  if (!motionQueueEntry->IsAvailable() || motionQueueEntry->IsFinished()) {
    return;
  }
  ...
  motionQueueEntry->SetStartTime(userTimeSeconds -
                                 _offsetSeconds);  // モーションの開始時刻を記録
  motionQueueEntry->SetFadeInStartTime(
      userTimeSeconds);  // フェードインの開始時刻
...
csmFloat32 ACubismMotion::UpdateFadeWeight(
    CubismMotionQueueEntry *motionQueueEntry, csmFloat32 userTimeSeconds) {
...
csmFloat32 ACubismMotion::GetFadeInTime() const { return this->_fadeInSeconds; }

void ACubismMotion::SetWeight(csmFloat32 weight) { this->_weight = weight; }

csmFloat32 ACubismMotion::GetWeight() const { return this->_weight; }

csmFloat32 ACubismMotion::GetDuration() { return -1.0f; }

csmFloat32 ACubismMotion::GetLoopDuration() { return -1.0f; }

void ACubismMotion::SetOffsetTime(csmFloat32 offsetSeconds) {
  this->_offsetSeconds = offsetSeconds;
}

const csmVector<const csmString *> &ACubismMotion::GetFiredEvent(
    csmFloat32 beforeCheckTimeSeconds, csmFloat32 motionTimeSeconds) {
  return _firedEventValues;
}

void ACubismMotion::SetFinishedMotionHandler(
    FinishedMotionCallback onFinishedMotionHandler) {
  this->_onFinishedMotion = onFinishedMotionHandler;
}

ACubismMotion::FinishedMotionCallback
ACubismMotion::GetFinishedMotionHandler() {
  return this->_onFinishedMotion;
}

void ACubismMotion::SetFinishedMotionCustomData(
    void *onFinishedMotionCustomData) {
...
void ACubismMotion::SetFinishedMotionHandlerAndMotionCustomData(
    FinishedMotionCallback onFinishedMotionHandler,
    void *onFinishedMotionCustomData) {
  this->_onFinishedMotion = onFinishedMotionHandler;
  this->_onFinishedMotionCustomData = onFinishedMotionCustomData;
...
csmBool ACubismMotion::IsExistModelOpacity() const { return false; }

csmInt32 ACubismMotion::GetModelOpacityIndex() const { return -1; }

CubismIdHandle ACubismMotion::GetModelOpacityId(csmInt32 index) { return NULL; }

csmFloat32 ACubismMotion::GetModelOpacityValue() const { return 1.0f; }
