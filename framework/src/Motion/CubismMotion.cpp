...
/**
 * Cubism SDK R2 以前のモーションを再現させるなら true
 * 、アニメータのモーションを正しく再現するなら false 。
 */
const csmBool UseOldBeziersCurveMotion = false;

CubismMotionPoint LerpPoints(const CubismMotionPoint a,
                             const CubismMotionPoint b, const csmFloat32 t) {
...
csmFloat32 LinearEvaluate(const CubismMotionPoint *points,
...
csmFloat32 BezierEvaluate(const CubismMotionPoint *points,
                          const csmFloat32 time) {
...
csmFloat32 BezierEvaluateBinarySearch(const CubismMotionPoint *points,
                                      const csmFloat32 time) {
...
csmFloat32 BezierEvaluateCardanoInterpretation(const CubismMotionPoint *points,
                                               const csmFloat32 time) {
  const csmFloat32 x = time;
  csmFloat32 x1 = points[0].Time;
  csmFloat32 x2 = points[3].Time;
...
csmFloat32 SteppedEvaluate(const CubismMotionPoint *points,
                           const csmFloat32 time) {
  return points[0].Value;
}

csmFloat32 InverseSteppedEvaluate(const CubismMotionPoint *points,
                                  const csmFloat32 time) {
  return points[1].Value;
}

csmFloat32 EvaluateCurve(const CubismMotionData *motionData,
                         const csmInt32 index, csmFloat32 time) {
  // Find segment to evaluate.
  const CubismMotionCurve &curve = motionData->Curves[index];

  csmInt32 target = -1;
  const csmInt32 totalSegmentCount =
...
CubismMotion::~CubismMotion() { CSM_DELETE(_motionData); }

CubismMotion *CubismMotion::Create(
    const csmByte *buffer, csmSizeInt size,
...
void CubismMotion::DoUpdateParameters(
    CubismModel *model, csmFloat32 userTimeSeconds, csmFloat32 fadeWeight,
  ...
  if (_eyeBlinkParameterIds.GetSize() > MaxTargetSize) {
    CubismLogDebug("too many eye blink targets : %d",
                   _eyeBlinkParameterIds.GetSize());
  }
  if (_lipSyncParameterIds.GetSize() > MaxTargetSize) {
    CubismLogDebug("too many lip sync targets : %d",
                   _lipSyncParameterIds.GetSize());
  }
  ...
  for (c = 0; c < _motionData->CurveCount &&
              curves[c].Type == CubismMotionCurveTarget_Model;
       ++c) {
  ...
  for (; c < _motionData->CurveCount &&
         curves[c].Type == CubismMotionCurveTarget_Parameter;
       ++c) {
    ...
    if (eyeBlinkValue != FLT_MAX) {
      for (csmUint32 i = 0;
           i < _eyeBlinkParameterIds.GetSize() && i < MaxTargetSize; ++i) {
    ...
    if (lipSyncValue != FLT_MAX) {
      for (csmUint32 i = 0;
           i < _lipSyncParameterIds.GetSize() && i < MaxTargetSize; ++i) {
    ...
    if (curves[c].FadeInTime < 0.0f && curves[c].FadeOutTime < 0.0f) {
      ...
      if (curves[c].FadeInTime < 0.0f) {
        ...
        fin = curves[c].FadeInTime == 0.0f
                  ...
                  : CubismMath::GetEasingSine(
                        (userTimeSeconds -
                         motionQueueEntry->GetFadeInStartTime()) /
                        curves[c].FadeInTime);
      }
      ...
      if (curves[c].FadeOutTime < 0.0f) {
        ...
        fout = (curves[c].FadeOutTime == 0.0f ||
                motionQueueEntry->GetEndTime() < 0.0f)
                   ? 1.0f
                   : CubismMath::GetEasingSine(
                         (motionQueueEntry->GetEndTime() - userTimeSeconds) /
...

  {
    if (eyeBlinkValue != FLT_MAX) {
      for (csmUint32 i = 0;
           i < _eyeBlinkParameterIds.GetSize() && i < MaxTargetSize; ++i) {
        const csmFloat32 sourceValue =
            model->GetParameterValue(_eyeBlinkParameterIds[i]);
    ...
    if (lipSyncValue != FLT_MAX) {
      for (csmUint32 i = 0;
           i < _lipSyncParameterIds.GetSize() && i < MaxTargetSize; ++i) {
        const csmFloat32 sourceValue =
            model->GetParameterValue(_lipSyncParameterIds[i]);
  ...
  for (; c < _motionData->CurveCount &&
         curves[c].Type == CubismMotionCurveTarget_PartOpacity;
       ++c) {
    // Find parameter index.
    parameterIndex = model->GetParameterIndex(curves[c].Id);
...
void CubismMotion::Parse(const csmByte *motionJson, const csmSizeInt size) {
  ...
  csmBool areBeziersRestricted =
      json->GetEvaluationOptionFlag(EvaluationOptionFlag_AreBeziersRestricted);
  ...
  _motionData->Curves.UpdateSize(_motionData->CurveCount, CubismMotionCurve(),
                                 true);
  _motionData->Segments.UpdateSize(json->GetMotionTotalSegmentCount(),
                                   CubismMotionSegment(), true);
  _motionData->Points.UpdateSize(json->GetMotionTotalPointCount(),
                                 CubismMotionPoint(), true);
  _motionData->Events.UpdateSize(_motionData->EventCount, CubismMotionEvent(),
                                 true);
  ...
  // Curves
  for (csmInt32 curveCount = 0; curveCount < _motionData->CurveCount;
       ++curveCount) {
    ...
    } else if (strcmp(json->GetMotionCurveTarget(curveCount),
                      TargetNameParameter) == 0) {
      _motionData->Curves[curveCount].Type = CubismMotionCurveTarget_Parameter;
    } else if (strcmp(json->GetMotionCurveTarget(curveCount),
                      TargetNamePartOpacity) == 0) {
      _motionData->Curves[curveCount].Type =
          CubismMotionCurveTarget_PartOpacity;
    } else {
      CubismLogWarning(
          "Warning : Unable to get segment type from Curve! The number "
          "of \"CurveCount\" may be incorrect!");
    }
    ...
    // Segments
    for (csmInt32 segmentPosition = 0;
         segmentPosition < json->GetMotionCurveSegmentCount(curveCount);) {
      if (segmentPosition == 0) {
        _motionData->Segments[totalSegmentCount].BasePointIndex =
            totalPointCount;

        _motionData->Points[totalPointCount].Time =
            json->GetMotionCurveSegment(curveCount, segmentPosition);
        _motionData->Points[totalPointCount].Value =
      ...
      const csmInt32 segment = static_cast<csmInt32>(
          json->GetMotionCurveSegment(curveCount, segmentPosition));
      ...
      switch (segment) {
        ...
        case CubismMotionSegmentType_Bezier: {
          ...
          if (areBeziersRestricted || UseOldBeziersCurveMotion) {
            ...
            _motionData->Segments[totalSegmentCount].Evaluate =
          ...
          _motionData->Points[totalPointCount + 1].Time =
              json->GetMotionCurveSegment(curveCount, (segmentPosition + 3));
          _motionData->Points[totalPointCount + 1].Value =
              json->GetMotionCurveSegment(curveCount, (segmentPosition + 4));

          _motionData->Points[totalPointCount + 2].Time =
              json->GetMotionCurveSegment(curveCount, (segmentPosition + 5));
          _motionData->Points[totalPointCount + 2].Value =
  ...
  for (csmInt32 userdatacount = 0; userdatacount < json->GetEventCount();
       ++userdatacount) {
    _motionData->Events[userdatacount].FireTime =
        json->GetEventTime(userdatacount);
    _motionData->Events[userdatacount].Value =
...
void CubismMotion::SetParameterFadeInTime(CubismIdHandle parameterId,
                                          csmFloat32 value) {
  csmVector<CubismMotionCurve> &curves = _motionData->Curves;
...
void CubismMotion::SetParameterFadeOutTime(CubismIdHandle parameterId,
                                           csmFloat32 value) {
  csmVector<CubismMotionCurve> &curves = _motionData->Curves;
...
csmFloat32 CubismMotion::GetParameterFadeInTime(
    CubismIdHandle parameterId) const {
  csmVector<CubismMotionCurve> &curves = _motionData->Curves;
...
csmFloat32 CubismMotion::GetParameterFadeOutTime(
    CubismIdHandle parameterId) const {
  csmVector<CubismMotionCurve> &curves = _motionData->Curves;
...
void CubismMotion::IsLoop(csmBool loop) { this->_isLoop = loop; }

csmBool CubismMotion::IsLoop() const { return this->_isLoop; }

void CubismMotion::IsLoopFadeIn(csmBool loopFadeIn) {
  this->_isLoopFadeIn = loopFadeIn;
}

csmBool CubismMotion::IsLoopFadeIn() const { return this->_isLoopFadeIn; }

csmFloat32 CubismMotion::GetLoopDuration() { return _loopDurationSeconds; }

void CubismMotion::SetEffectIds(
    const csmVector<CubismIdHandle> &eyeBlinkParameterIds,
...
const csmVector<const csmString *> &CubismMotion::GetFiredEvent(
    ...
    if (curve.Type == CubismMotionCurveTarget_Model) {
      if (strcmp(curve.Id->GetString().GetRawString(), IdNameOpacity) == 0) {
        return CubismFramework::GetIdManager()->GetId(
            curve.Id->GetString().GetRawString());
      }
    }
  }
...
csmFloat32 CubismMotion::GetModelOpacityValue() const { return _modelOpacity; }
