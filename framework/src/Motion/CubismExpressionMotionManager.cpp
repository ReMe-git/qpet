...
CubismExpressionMotionManager::CubismExpressionMotionManager()
    : _currentPriority(0),
      ...
      _expressionParameterValues(
          CSM_NEW csmVector<ExpressionParameterValue>()) {}
...
CubismMotionQueueEntryHandle CubismExpressionMotionManager::StartMotionPriority(
    ACubismMotion *motion, csmBool autoDelete, csmInt32 priority) {
...
csmBool CubismExpressionMotionManager::UpdateMotion(
    CubismModel *model, csmFloat32 deltaTimeSeconds) {
  ...
  for (csmVector<CubismMotionQueueEntry *>::iterator ite = motions->Begin();
       ite != motions->End();) {
    ...
    CubismExpressionMotion *expressionMotion =
        (CubismExpressionMotion *)motionQueueEntry->GetCubismMotion();
    ...
    csmVector<CubismExpressionMotion::ExpressionParameter>
        expressionParameters = expressionMotion->GetExpressionParameters();
    if (motionQueueEntry->IsAvailable()) {
      ...
      for (csmInt32 i = 0; i < expressionParameters.GetSize(); ++i) {
        ...
        for (csmInt32 j = 0; j < _expressionParameterValues->GetSize(); ++j) {
          if (_expressionParameterValues->At(j).ParameterId !=
    ...
    _fadeWeights[expressionIndex] =
        expressionMotion->UpdateFadeWeight(motionQueueEntry, _userTimeSeconds);
    expressionMotion->CalculateExpressionParameters(
        model, _userTimeSeconds, motionQueueEntry, _expressionParameterValues,
        expressionIndex, _fadeWeights[expressionIndex]);

    expressionWeight +=
        expressionMotion->GetFadeInTime() == 0.0f
            ? 1.0f
            : CubismMath::GetEasingSine(
                  (_userTimeSeconds - motionQueueEntry->GetFadeInStartTime()) /
  ...
  if (motions->GetSize() > 1) {
    CubismExpressionMotion *expressionMotion =
        (CubismExpressionMotion *)(motions->At(motions->GetSize() - 1))
            ->GetCubismMotion();
  ...
  for (csmInt32 i = 0; i < _expressionParameterValues->GetSize(); ++i) {
    model->SetParameterValue(
        _expressionParameterValues->At(i).ParameterId,
        (_expressionParameterValues->At(i).OverwriteValue +
         _expressionParameterValues->At(i).AdditiveValue) *
            _expressionParameterValues->At(i).MultiplyValue,
        expressionWeight);
