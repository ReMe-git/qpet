...
CubismMoc *CubismMoc::Create(const csmByte *mocBytes, csmSizeInt size,
  ...
  const Core::csmMocVersion version =
      Core::csmGetMocVersion(alignedBuffer, size);
...
void CubismMoc::Delete(CubismMoc *moc) { CSM_DELETE_SELF(CubismMoc, moc); }

CubismMoc::CubismMoc(Core::csmMoc *moc)
    : _moc(moc), _modelCount(0), _mocVersion(0) {}
...
CubismModel *CubismMoc::CreateModel() {
  ...
  Core::csmModel *model =
      Core::csmInitializeModelInPlace(_moc, modelMemory, modelSize);
...
Core::csmMocVersion CubismMoc::GetMocVersion() { return _mocVersion; }
...
csmBool CubismMoc::HasMocConsistencyFromUnrevivedMoc(const csmByte *mocBytes,
                                                     csmSizeInt size) {
  void *alignedBuffer = CSM_MALLOC_ALLIGNED(size, Core::csmAlignofMoc);
  memcpy(alignedBuffer, mocBytes, size);
