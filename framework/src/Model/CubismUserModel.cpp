...
void CubismUserModel::SetAcceleration(csmFloat32 x, csmFloat32 y,
                                      csmFloat32 z) {
  _accelerationX = x;
  _accelerationY = y;
  _accelerationZ = z;
...
void CubismUserModel::LoadModel(const csmByte *buffer, csmSizeInt size,
                                csmBool shouldCheckMocConsistency) {
...
ACubismMotion *CubismUserModel::LoadExpression(const csmByte *buffer,
                                               csmSizeInt size,
                                               const csmChar *name) {
  if (!buffer) {
...
csmBool CubismUserModel::IsHit(CubismIdHandle drawableId, csmFloat32 pointX,
                               csmFloat32 pointY) {
  ...
  for (csmInt32 j = 1; j < count; ++j) {
    ...
    csmFloat32 y =
        vertices[Constant::VertexOffset + j * Constant::VertexStep + 1];
...
ACubismMotion *CubismUserModel::LoadMotion(
    const csmByte *buffer, csmSizeInt size, const csmChar *name,
...
csmBool CubismUserModel::IsInitialized() { return _initialized; }

void CubismUserModel::IsInitialized(csmBool v) { _initialized = v; }

csmBool CubismUserModel::IsUpdating() { return _updating; }

void CubismUserModel::IsUpdating(csmBool v) { _updating = v; }

void CubismUserModel::SetOpacity(csmFloat32 a) { _opacity = a; }

csmFloat32 CubismUserModel::GetOpacity() { return _opacity; }

CubismModel *CubismUserModel::GetModel() const { return _model; }
...
void CubismUserModel::CubismDefaultMotionEventCallback(
    const CubismMotionQueueManager *caller, const csmString &eventValue,
    void *customData) {
