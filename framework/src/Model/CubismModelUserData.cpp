...
const csmVector<const CubismModelUserData::CubismModelUserDataNode *> &
CubismModelUserData::GetArtMeshUserDatas() const {
  return _artMeshUserDataNodes;
}

CubismModelUserData *CubismModelUserData::Create(const csmByte *buffer,
                                                 const csmSizeInt size) {
...
void CubismModelUserData::ParseUserData(const csmByte *buffer,
                                        const csmSizeInt size) {
  ...
  const ModelUserDataType typeOfArtMesh =
      CubismFramework::GetIdManager()->GetId(ArtMesh);
  ...
  for (csmUint32 i = 0; i < nodeCount; i++) {
    ...
    addNode->TargetType =
        CubismFramework::GetIdManager()->GetId(json->GetUserDataTargetType(i));
